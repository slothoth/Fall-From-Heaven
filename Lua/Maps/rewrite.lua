...
function assignStartingPlots()
    ...
    playerList = {}
    for plrCheckLoop=1, maxCivPlayers do
    ...
    for _, playerIndex in ipairs(playerList) do
        local player = gc.getPlayer(playerIndex)                -- bleh need custom
        player.AI_updateFoundValues(true)                   -- no clue what this does
        local civType = player.getCivilizationType()            -- convert
        local civInfo = gc.getCivilizationInfo(civType)         -- convert
        print("Civ = %s", civInfo.getType())
...
function getStartPlotInRegion(region,player,civPref)
    ...
    for _, plot in ipairs(region.plotList) do
        ...
        if not(civPref.needCoastalStart and not isCoast(startPlot)) and not startPlot.isPeak() then
...
function replaceUniqueImprovements()
    ...
    for y in range(mapSize.MapHeight) do
        for x in range(mapSize.MapWidth) do
...
function createChokePointList()
    ...
    for y=1, mapSize.MapHeight do
        for x=1, mapSize.MapWidth do
            i = GetIndex(x,y)
