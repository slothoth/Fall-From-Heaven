...
local tTraitPropKeys = {
        SELECTED_AGGRESSIVE='SLTH_TRAIT_AGGRESSIVE',
        SELECTED_ARCANE='SLTH_TRAIT_ARCANE',
        SELECTED_CHARISMATIC='SLTH_TRAIT_CHARISMATIC',
        SELECTED_CREATIVE='SLTH_TRAIT_CREATIVE',
        SELECTED_DEFENDER='SLTH_TRAIT_DEFENDER',
        SELECTED_EXPANSIVE='SLTH_TRAIT_EXPANSIVE',
        SELECTED_FINANCIAL='SLTH_TRAIT_FINANCIAL',
        SELECTED_INDUSTRIOUS='SLTH_TRAIT_INDUSTRIOUS',
        SELECTED_ORGANIZED='SLTH_TRAIT_ORGANIZED',
        SELECTED_PHILOSOPHICAL='SLTH_TRAIT_PHILOSOPHICAL',
        SELECTED_RAIDERS='SLTH_TRAIT_RAIDERS',
        SELECTED_SPIRITUAL='SLTH_TRAIT_SPIRITUAL',
        SELECTED_SUMMONER='SLTH_TRAIT_SUMMONER'
    }
-- MODDED END
function GetLeaderUniqueTraits( leaderType:string, useFullDescriptions:boolean )
    print('in new leader Unique traits')
	-- Gather info.
    ...
    for row in GameInfo.LeaderTraits() do
        if(has_leader[row.LeaderType] == true) then
			...
			if(trait) then
				...
				-- modded section
				if row.TraitType == 'SLTH_TRAIT_INSANE' then
					if Game and Game.GetCurrentGameTurn() ~= 1 then
						-- find player with this leadertype, if plural, just the first because HEADACHE
						local pInsanePlayer
						for iPlayer, pPlayer in ipairs(Players) do
							if not pInsanePlayer and pPlayer:IsMajor() and pPlayer:IsAlive() then		-- priority first
								local pConfig = PlayerConfigurations[iPlayer]
								if pConfig and pConfig:GetLeaderTypeName() == 'LEADER_PERPENTACH' then
									pInsanePlayer = pPlayer
								end
							end
						end
						...
						table.insert(traits, GameInfo.Traits['SLTH_TRAIT_ARCANE']);
						table.insert(traits, GameInfo.Traits['SLTH_TRAIT_CHARISMATIC']);
						table.insert(traits, GameInfo.Traits['SLTH_TRAIT_CREATIVE']);
					end
				end
			end
