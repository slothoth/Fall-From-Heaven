...
--	Override for seeing possible buildings in districts.
-- ===========================================================================
include("ProductionPanel");
print('loading Production Panel Override')
-- Override. We need to show buildings that arent buildable, if you dont have the district, since district is free
local tNoBuildDistricts = {['DISTRICT_WONDER']=true, ['DISTRICT_CITY_CENTER']=true}

local tBuildingsPrereqDistrictAndTech = {}
local tBuildingsPrereqDistrictAndCivic = {}
for row in GameInfo.Buildings() do
	if row.PrereqDistrict and not tNoBuildDistricts[row.PrereqDistrict] then
		if row.PrereqTech then
			tBuildingsPrereqDistrictAndTech[row.BuildingType] = GameInfo.Technologies[row.PrereqTech].Index
		end
		if row.PrereqCivic then
			tBuildingsPrereqDistrictAndCivic[row.BuildingType] = GameInfo.Civics[row.PrereqCivic].Index
		end
	end
end

local validFails = {['This building requires a Campus district.']=true,
					['This building requires a Theater Square district.']=true,
					['This building requires an Industrial Zone district.']=true,
					['This building requires a Commercial Hub district.']=true,
					['This building requires a Holy Site district.']=true
}
...
function GetData()
	...
	for row in GameInfo.Buildings() do
		...
		if row.PrereqDistrict ~= nil then
			...
			if not tNoBuildDistricts[iPrereqDistrict] and not row.MustPurchase then
				local earlyCanStart, earlyResults = buildQueue:CanProduce( row.Hash, false, true );
				if earlyResults and not earlyCanStart then
					local sFailString = earlyResults[CityCommandResults.FAILURE_REASONS]
					if sFailString and table.count(sFailString) > 0 then
						if validFails[earlyResults[CityCommandResults.FAILURE_REASONS][1]] then
							if row.PrereqTech then
								local iTechReq = tBuildingsPrereqDistrictAndTech[row.BuildingType]
								bCanProduce = pTechs:HasTech(iTechReq)
							end
							if row.PrereqCivic then
								local iCivicReq = tBuildingsPrereqDistrictAndCivic[row.BuildingType]
								bCanProduce = pCulture:HasCivic(iCivicReq)
							end
						end
					end
				end
