...
local ROW_INDEX :table = {
	...
	NEWSLOT1 = 5, -- Zega: These are the slot Types that you can slot Policies in, so not like Great Person Policies who don't have a specific slot. So if you're adding a new type of Slot that doesn't hold Policies, but only serves as a new Policy Type that is slotted in Wildcard Slot, then it shouldn't be added here. each additional slot gets +1 Index
	NEWSLOT2 = 6, -- Zega: 5+1
...
local ROW_SLOT_TYPES :table = {};
	...
	ROW_SLOT_TYPES[ROW_INDEX.NEWSLOT1] = "SLOT_NEWSLOT1"; -- Zega: SLOT_NEWSLOT1 is the new GovernmentSlotType from the Database.
	ROW_SLOT_TYPES[ROW_INDEX.NEWSLOT2] = "SLOT_NEWSLOT2"; -- Zega
...
local SLOT_ORDER_IN_CATALOG :table = {
	...
	SLOT_NEWSLOT1	= 6, -- Zega: you can rearange the order in which the slots get organized in the Policies Catalog as you want. You can also add Policy Slots like the Great Person Slot here.
	SLOT_NEWSLOT2	= 7, -- Zega
...
local PIC_CARD_TYPE_SLOT_NEWSLOT1	:string = "Governments_Newslot1Card"; -- Zega: The Texture of the new SlotType
local PIC_CARD_TYPE_SLOT_NEWSLOT2	:string = "Governments_Newslot2Card"; -- Zega
...
local PICS_SLOT_TYPE_CARD_BGS		:table  = {
	...
	SLOT_NEWSLOT1  = PIC_CARD_TYPE_SLOT_NEWSLOT1;  -- Zega: You can also add Policy Slots like the Great Person Slot here, just make sure it's utilized as a wild card, like: SLOT_NEWSLOT3  = PIC_CARD_TYPE_WILDCARD;
	SLOT_NEWSLOT2  = PIC_CARD_TYPE_SLOT_NEWSLOT2;  -- Zega
...
local IMG_POLICYCARD_BY_ROWIDX :table = {};
	...
	IMG_POLICYCARD_BY_ROWIDX[ROW_INDEX.NEWSLOT1] = PIC_CARD_TYPE_SLOT_NEWSLOT1; -- Zega: from ROW_INDEX and PICS_SLOT_TYPE_CARD_BGS
	IMG_POLICYCARD_BY_ROWIDX[ROW_INDEX.NEWSLOT2] = PIC_CARD_TYPE_SLOT_NEWSLOT2; -- Zega
...
function IsSlotTypeLegalInRow( nRowIndex:number, strSlotType:string )
	...
	-- Zega: In case we have a New Slot type that doesn't hold Policies just like Great Person Slot, then it must be added like This (comment the line above and uncomment the next code line):
	--if strSlotType == "SLOT_GREAT_PERSON" or strSlotType == "SLOT_NEWSLOT3" or strSlotType == "SLOT_NEWSLOT4" then strSlotType = "SLOT_WILDCARD"; end
...
function RealizeGovernmentInstance(governmentType:string, inst:table, isCivilopediaAvailable:boolean)
	...
	-->>> Zega
	for i=1,government.NumSlotNewSlot1,1 do -- NumSlotNewSlot1 is from PopulateStaticData() function down below, whatever it's named there it must be reflected here
		totalSlotsSize = totalSlotsSize + CreateMinSlotInstance(inst.SlotStack, PIC_CARD_TYPE_SLOT_NEWSLOT1);
	end
	totalSlots = totalSlots + government.NumSlotNewSlot1;
	
	for i=1,government.NumSlotNewSlot2,1 do
		totalSlotsSize = totalSlotsSize + CreateMinSlotInstance(inst.SlotStack, PIC_CARD_TYPE_SLOT_NEWSLOT2);
	end
	totalSlots = totalSlots + government.NumSlotNewSlot2;
	--<<< Zega
...
function BlockRowsUnableToAccept( strSlotType:string )
	...
	if ( strSlotType == nil ) then
		...
		Controls.NewSlot1Blocker:SetHide(true); -- Zega: NewSlot1Blocker must be same Name as in the XML file
		Controls.NewSlot2Blocker:SetHide(true); -- Zega
	...
	Controls.NewSlot1Blocker:SetHide(IsSlotTypeLegalInRow( ROW_INDEX.NEWSLOT1, strSlotType )); -- Zega: NewSlot1Blocker must be same Name as in the XML file, NEWSLOT1 is from ROW_INDEX at the top of the file
	Controls.NewSlot2Blocker:SetHide(IsSlotTypeLegalInRow( ROW_INDEX.NEWSLOT2, strSlotType )); -- Zega
...
function RealizeActivePoliciesRows()
	...
	Controls.StackNewSlot1:DestroyAllChildren(); -- Zega: StackNewSlot1 must be same name as in the XML file.
	Controls.StackNewSlot2:DestroyAllChildren(); -- Zega
	...
	tStackControls[ROW_INDEX.NEWSLOT1] = Controls.StackNewSlot1; -- Zega: StackNewSlot1 must be same name as in the XML file.
	tStackControls[ROW_INDEX.NEWSLOT2] = Controls.StackNewSlot2; -- Zega
	...
	tRowCtrlPrefix[ROW_INDEX.NEWSLOT1] = "NewSlot1"; -- Zega: "NewSlot1" String naming must reflect whatever is in "[New Slot Type String]CardSlots" section in the XML, mainly "[String]LabelRight", "[String]LabelLeft", "[String]Counter", "[String]IconRing", "[String]IconLeft".
	tRowCtrlPrefix[ROW_INDEX.NEWSLOT2] = "NewSlot2"; -- Zega
	...
	tRowTooltip[ROW_INDEX.NEWSLOT1] = "LOC_GOVT_POLICY_TYPE_NEWSLOT1"; -- Zega: Tooltip String of the New Slot Type
	tRowTooltip[ROW_INDEX.NEWSLOT2] = "LOC_GOVT_POLICY_TYPE_NEWSLOT2"; -- Zega
	...
	Controls.NewSlot1Empty:SetHide(#m_ActivePolicyRows[ROW_INDEX.NEWSLOT1].SlotArray > 0); -- Zega: NewSlot1Empty must be same name as in the XML
	Controls.NewSlot2Empty:SetHide(#m_ActivePolicyRows[ROW_INDEX.NEWSLOT2].SlotArray > 0); -- Zega
...
function RealizeActivePolicyRowSize()
	...
	EnsureRowContentsFit(ROW_INDEX.NEWSLOT1, Controls.StackNewSlot1); -- Zega: StackNewSlot1 must be same Name as in the XML
	EnsureRowContentsFit(ROW_INDEX.NEWSLOT2, Controls.StackNewSlot2); -- Zega
...
function OnConfirmPolicies()
	...
	if Controls.ConfirmPolicies:IsDisabled() then
		...
		PulseAnim(Controls.NewSlot1IconRingAnim); -- Zega: NewSlot1IconRingAnim must be same name as in the XML
		PulseAnim(Controls.NewSlot2IconRingAnim); -- Zega
...
function SwitchTab( tabFrom, tabTo, bForce )
	...
	Controls.NewSlot1Counter:SetHide( not isToPolicies ); -- Zega: NewSlot1Counter Must be same name as in the XML
	Controls.NewSlot2Counter:SetHide( not isToPolicies ); -- Zega
...
function GetCurrentDragTargetRowIndex( pDraggedControl:table, strPolicyType:string )
	...
	-->>> Zega: This handles where the dragged Policy can be slotted in, if its Slot type is NEWSLOT1 then it should go in that Slot, else in the Wildcard Slot.
	-- RowNewSlot1 must be same name as in the XML file
	if ( IsPolicyTypeLegalInRow( ROW_INDEX.NEWSLOT1, strPolicyType ) ) then
		table.insert( kViableDragTargets, Controls.RowNewSlot1 );
		kTargetToTypeMap[Controls.RowNewSlot1] = ROW_INDEX.NEWSLOT1;
	end
	if ( IsPolicyTypeLegalInRow( ROW_INDEX.NEWSLOT2, strPolicyType ) ) then
		table.insert( kViableDragTargets, Controls.RowNewSlot2 );
		kTargetToTypeMap[Controls.RowNewSlot2] = ROW_INDEX.NEWSLOT2;
	end
	--<<< Zega
...
function IsDragOverRow( pDraggedControl:table )
	...
	table.insert( kTargets, Controls.RowNewSlot1 ); -- Zega: RowNewSlot1 must be same name as in the XML file
	table.insert( kTargets, Controls.RowNewSlot2 ); -- Zega
	...
	if not TryToAddToRow( ROW_INDEX.MILITARY, cardSlotType, policyType ) then
		if not TryToAddToRow( ROW_INDEX.ECONOMIC, cardSlotType, policyType ) then
			if not TryToAddToRow( ROW_INDEX.DIPLOMAT, cardSlotType, policyType ) then
				-->>> Zega: Add a new if statement inside the other (except in wildcard) for each new Government Slot Type that you slot policies in (unlike GPP Slot types).
				if not TryToAddToRow( ROW_INDEX.NEWSLOT1, cardSlotType, policyType ) then
					if not TryToAddToRow( ROW_INDEX.NEWSLOT2, cardSlotType, policyType ) then
					--<<< Zega
...
function PopulateLivePlayerData( ePlayer:number )
	...
	m_ActivePolicyRows[ROW_INDEX.NEWSLOT1] = { SlotArray={} }; -- Zega
	m_ActivePolicyRows[ROW_INDEX.NEWSLOT2] = { SlotArray={} }; -- Zega
...
function PopulateStaticData()
	...
	for row in GameInfo.Governments() do
		...
		local slotNewSlot1		:number = 0; -- Zega
		local slotNewSlot2		:number = 0; -- Zega
		...
		for entry in GameInfo.Government_SlotCounts() do
			if row.GovernmentType == entry.GovernmentType then
				...
				for i = 1, entry.NumSlots, 1 do
					...
					elseif	slotType == "SLOT_NEWSLOT1" then								slotNewSlot1	= slotNewSlot1 + 1; -- Zega: SLOT_NEWSLOT1 is the GovernmentSlotType in the DB.
					elseif	slotType == "SLOT_NEWSLOT2" then								slotNewSlot2	= slotNewSlot2 + 1; -- Zega
					
					elseif	slotType == "SLOT_WILDCARD" or slotType=="SLOT_GREAT_PERSON" then	slotWildcard	= slotWildcard + 1;
					
					-- Zega: In case the new Policy Slot is like the Great Person slot, it needs to be added like this (uncomment the next code line and comment the code line above this one):
					--elseif	slotType == "SLOT_WILDCARD" or slotType=="SLOT_GREAT_PERSON" or slotType=="SLOT_NEWSLOT3" or slotType=="SLOT_NEWSLOT4" then	slotWildcard	= slotWildcard + 1;
		...
		g_kGovernments[row.GovernmentType] = {
			...
			NumSlotNewSlot1		= slotNewSlot1, -- Zega
			NumSlotNewSlot2		= slotNewSlot2, -- Zega
...
function newSlot1Filter(policy)	return policy.SlotType == "SLOT_NEWSLOT1"; end -- Zega
function newSlot2Filter(policy)	return policy.SlotType == "SLOT_NEWSLOT2"; end -- Zega

function wildcardFilter(policy)		return policy.SlotType == "SLOT_WILDCARD" or policy.SlotType == "SLOT_GREAT_PERSON" end

-- Zega: In case we have a new Policy Slot that is like the Great Person slot, it needs to be added like this (uncomment the next code line and comment the code line above this one):
--function wildcardFilter(policy)		return policy.SlotType == "SLOT_WILDCARD" or policy.SlotType == "SLOT_GREAT_PERSON" or policy.SlotType == "SLOT_NEWSLOT3" or policy.SlotType == "SLOT_NEWSLOT4" end 
...
function PopulatePolicyFilterData()
	...
	table.insert( m_kPolicyFilters, { Func=newSlot1Filter,		Description="LOC_GOVT_FILTER_NEWSLOT1"	} ); -- Zega
	table.insert( m_kPolicyFilters, { Func=newSlot2Filter,		Description="LOC_GOVT_FILTER_NEWSLOT2"	} ); -- Zega
...
function Initialize()
	...
	Controls.LabelNewSlot1:SetText(Locale.Lookup("{LOC_GOVT_POLICY_TYPE_NEWSLOT1:upper}")); -- Zega: LabelNewSlot1 must be the same Name as in the XML
	Controls.LabelNewSlot2:SetText(Locale.Lookup("{LOC_GOVT_POLICY_TYPE_NEWSLOT2:upper}")); -- Zega
