-------------------------
-- Civ 6 Map Utilities, edited GoodyHut stuff --
-------------------------
...
function CanPlaceGoodyAt(improvement, plot)
    ...
    if improvement.BarbarianCamp ~= 1 then                  -- modded change, barb camps can spawn near
        for dx = -3, 3 do
            for dy = -3, 3 do
...
function AddGoodies(iW, iH)
	...
	for improvement in GameInfo.Improvements() do
		local improvementID = improvement.RowId - 1;
		if(improvement.TilesPerGoody and improvement.GoodyRange) then                           -- modded change
			for x = 0, iW - 1 do
				for y = 0, iH - 1 do
					local i = y * iW + x;
					local pPlot = Map.GetPlotByIndex(i);
