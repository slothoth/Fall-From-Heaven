...
class PythonRandom:
    ...
    def seed(self):
        ...
        if self.usePR and CyGame().isNetworkMultiPlayer():
            print
            "Detecting network game. Setting UsePythonRandom to False."
            self.usePR = False
        if self.usePR:
            ...
            seed(seedValue)
            print
            "Random seed (Using Python rands) for this map is %(s)20d" % {"s": seedValue}
        ...
        ##            print "Pre-set seed (Using Pyhon rands) for this map is %(s)20d" % {"s":seedValue}
            ...
            print
            "Random seed (Using getMapRand) for this map is %(s)20d" % {"s": seedValue}
...
class RegionMap:
    ...
    def createRegions(self):
        ...
        print
        "MapWidth = %(mw)d,MapHeight = %(mh)d" % {"mw": mapSize.MapWidth, "mh": mapSize.MapHeight}
        ...
        print
        "numTiles = %(n)d, numRegions = %(w)d" % {"n": numTiles, "w": numRegions}
        ...
        print
        "numTiles = %(n)d, numWaterRegions = %(w)d" % {"n": numTiles, "w": numWaterRegions}
    ...
    def PrintRegionList(self):
        print
        "Number of regions = %(n)d" % {"n": len(self.regionList)}
        for region in self.regionList:
            print
            str(region)
...
class Region:
    ...
    def __str__(self):
        string = "ID=%(id)d(%(c)s), size=%(s)d, altitude=%(a)d \n" % {"id": self.ID, "c": chr(self.ID + 33),
                                                                      "s": len(self.plotList), "a": self.altitude}
            ...
            if plot.bBorder == True:
                ...
                for direction in range(1, 5, 1):
                    ...
                    if ii != -1 and regMap.regionMap[ii] == neighborID:
                        ...
                        break
        print
        "borderPlotCount=%(bc)d" % {"bc": borderPlotCount}
    ...
    def createFlowMap(self):
        ...
        for region in regMap.regionList:
            ...
            if len(validGateList) == 0:
                ...
                "validGateList == 0!!!!!!!!!!!!!!!!!!!!"
                print
                "region = %(r)s" % {"r": str(region)}
                gRegion = regMap.getRegionByID(region.gateRegion)
                print
                "gateRegion = %(g)s" % {"g": str(gRegion)}
            ...
            for x in range(mapSize.MapWidth):
                ...
                if plotMap.plotMap[i] != plotMap.OCEAN:
                    ...
                    if rainFall < mc.DesertThreshold:
                        ...
                        if rainFall < ((PRand.random() * (
                                mc.PlainsThreshold - mc.DesertThreshold)) / 2.0) + mc.DesertThreshold + (
                                (mc.PlainsThreshold - mc.DesertThreshold) / 2.0):
                            self.terrainMap[i] = self.PLAINS
                        else:
                            self.terrainMap[i] = self.GRASS
                    else:
...
class Areamap:
    ...
    def scanAndFillLine(self, seg, areaID):
        ...
        for xLeftExtreme in range(seg.xLeft - landOffset, -1, -1):
            ...
            if debugReport:
                print
                "xLeftExtreme = %(xl)4d" % {'xl': xLeftExtreme}
        ...
        if debugReport:
            print
            "xLeftExtreme finally = %(xl)4d" % {'xl': xLeftExtreme}
            print
            "Going Right"
        ...
        for xRightExtreme in range(seg.xLeft, self.mapWidth, 1):
            if debugReport:
                print
                "xRightExtreme = %(xr)4d" % {'xr': xRightExtreme}
            ...
            if self.areaMap[i] == 0 and plotMap.plotMap[i] != plotMap.PEAK:
                ...
                if lineFound == False:
                    ...
                    if debugReport:
                        print
                        "starting new line at xLeftExtreme= %(xl)4d" % {'xl': xLeftExtreme}
                ...
                if debugReport:
                    print
                    "same direction to stack", str(newSeg)
                ...
                if xLeftExtreme < seg.xLeft or xRightExtreme >= seg.xRight:
                    ...
                    if debugReport:
                        print
                        "opposite direction to stack", str(newSeg)
        ...
        if lineFound == True:  # still a line needing to be put on stack
            if debugReport:
                print
                "still needing to stack some segs"
            lineFound = False
            # put same direction on stack
            newSeg = LineSegment(seg.y + seg.dy, xLeftExtreme, xRightExtreme - 1, seg.dy)
            self.segStack.append(newSeg)
...
class StartingPlotFinder:
    ...
    def getBestStartRegion(self, availableRegionList, occupiedRegionList, civPref):
        ...
        for startRegion in bestRegionList:
            ...
            weightedAverageDiff = ((civPref.altitudeWeight * altitudeDiff) + (
        ...
        print
        "chosen regionID = %(r)d" % {"r": startRegion.region.ID}
        return startRegion
    ...
    def replaceUniqueImprovements(self):
        ...
        for y in range(mapSize.MapHeight):
            for x in range(mapSize.MapWidth):
                ...
                impInfo = gc.getImprovementInfo(impType)
                print
                "Removing %(i)s at %(x)d, %(y)d" % {"i": impInfo.getType(), "x": x, "y": y}
                ...
                print
                "Adding %(i)s at %(x)d, %(y)d" % {"i": impInfo.getType(), "x": bestPlot.getX(), "y": bestPlot.getY()}
    ...
    def getBestImprovementRegion(self, availableRegionList, occupiedRegionList, impPref):
        ...
        for startRegion in bestRegionList:
            ...
            weightedAverageDiff = ((impPref.altitudeWeight * altitudeDiff) + distanceFactor + (
                        moistureDiff * impPref.moistureWeight)) / (impPref.altitudeWeight + impPref.moistureWeight + 1)
            print
            "regionID = %(r)d, altitudeDiff= %(ad)f, moistureDiff= %(md)f, altitudeWeight= %(aw)f, weightedAverageDiff= %(wd)f" % \
            {"r": startRegion.region.ID, "ad": altitudeDiff, "md": moistureDiff, "aw": impPref.altitudeWeight,
             "wd": weightedAverageDiff}
            startRegion.differenceFromIdeal = weightedAverageDiff
    ...
    def getBestImpPlotInRegion(self, region, impPref):
        ...
        for plot in region.plotList:
            ...
            print
            "gamePlot = %(x)d,%(y)d" % {"x": plot.x, "y": plot.y}
            ...
            if impPref.favoredTerrain != TerrainTypes.NO_TERRAIN \
                ...
                print
                "rejected for not favored terrain"
                continue
            if impPref.needChoke:
                ...
                if (plot.x >= chokePlot.getX() - 1 and plot.x <= chokePlot.getX() + 1 \
                    ...
                    print
                    "rejected not next to choke=%(x)d,%(y)d or not peak" % {"x": chokePlot.getX(),
                                                                            "y": chokePlot.getY()}
                    continue
    ...
    def replaceWatchtowers(self, count):
        ...
        print
        "towersPlacedAtChoke= %(c)d, towersPlacedInMiddle= %(m)d, total= %(t)d" % \
    ...
    def createChokePointList(self):
        ...
        for chokePoint in likelyChokeList:
            if self.isConfirmedChokePoint(chokePoint):
                print
                "Confirmed chokepoint at %(c)s" % {"c": str(chokePoint)}
                self.chokePointList.append(chokePoint)
            else:
                print
                "Rejected chokepoint at %(x)d, %(y)d" % {"x": chokePoint.x, "y": chokePoint.y}
    ...
    def canFindAdditionalAreaThroughChokes(self, possibleChoke, origionalArea, chokesCheckedList, bTopLayer):
        ...
        for neighborChoke in possibleChoke.neighborChokeList:
            ...
            if not alreadyChecked:
                ...
                largeAreaFound = self.canFindAdditionalAreaThroughChokes(neighborChoke, origionalArea,
                                                                         chokesCheckedList, False)
                ...
                for neighborChoke in neighborArea.neighborChokeList:
                    ...
                    if not alreadyChecked:
                        ...
                        if largeAreaFound == True:
                            ...
                            if bTopLayer:
                                possibleChoke.gateList.append(
                                    (neighborChoke.x, neighborChoke.y))  # for final path check
                            returnValue = True
...
def GetPlotAltitude(x, y):
    ...
    riverSubtract = riverSize * (
                (mc.RiverAltitudeSubtraction / riverAltRange) / float(regMap.highestRegionAltitude + 1))
...
def getGridSize(argsList):
    ...
    if seaLevel == GetInfoType("SEALEVEL_LOW"):
        scaler = math.sqrt(
            (mc.RegionsPerPlot - mc.WaterRegionsPerPlot) / (mc.RegionsPerPlot - mc.WaterRegionsPerPlot * 0.5))
        mc.WaterRegionsPerPlot = mc.WaterRegionsPerPlot * 0.5
        print
        "Sea level is low, map dimension scaler = %f" % scaler
    elif seaLevel == GetInfoType("SEALEVEL_HIGH"):
        scaler = math.sqrt(
            (mc.RegionsPerPlot - mc.WaterRegionsPerPlot) / (mc.RegionsPerPlot - mc.WaterRegionsPerPlot * 3.0))
        mc.WaterRegionsPerPlot = mc.WaterRegionsPerPlot * 2.0
        print
        "Sea level is high, map dimension scaler = %f" % scaler
    grid_sizes = {
