-- UnitOperations at the bottom, since they are always the same
CREATE TABLE IF NOT EXISTS CustomOperations(OperationType text primary key, Callback text, SimpleText text, SecondText text, PromotionPrereq text,
                                            UnitPrereq text, ActivationPrereq text, AbilityPrereq text, AlternateAbilityPrereq text,
                                            AlsoAbilityPrereq text, AlsoTwoAbilityPrereq text, BuildingPrereq text, DomainPrereq text, FormationPrereq text,
                                            TechPrereq text,
                                            SimpleAmount integer default 0, SecondAmount integer default 0,
                                            ThirdAmount integer default 0, DontShowOnDisabled default '0');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, PromotionPrereq) VALUES
('UNITOPERATION_SUMMON_AIR_ELEMENTAL', 'SlthOnSummon', 'SLTH_UNIT_AIR_ELEMENTAL', 'AIR_THREE'),
('UNITOPERATION_SUMMON_DJINN', 'SlthOnSummon', 'SLTH_UNIT_DJINN', 'METAMAGIC_THREE'),
('UNITOPERATION_SUMMON_EYE', 'SlthOnSummon', 'SLTH_UNIT_EYE', 'METAMAGIC_ONE'),
('UNITOPERATION_SUMMON_EARTH_ELEMENTAL', 'SlthOnSummon', 'SLTH_UNIT_EARTH_ELEMENTAL', 'EARTH_THREE'),
('UNITOPERATION_SUMMON_FIRE_ELEMENTAL', 'SlthOnSummon', 'SLTH_UNIT_FIRE_ELEMENTAL', 'FIRE_THREE'),
('UNITOPERATION_SUMMON_FIREBALL', 'SlthOnSummon', 'SLTH_UNIT_FIREBALL', 'FIRE_TWO'),
('UNITOPERATION_SUMMON_ICE_ELEMENTAL', 'SlthOnSummon', 'SLTH_UNIT_ICE_ELEMENTAL', 'ICE_TWO'),
('UNITOPERATION_SUMMON_EINHERJAR', 'SlthOnSummon', 'SLTH_UNIT_EINHERJAR', 'LAW_TWO'),
('UNITOPERATION_SUMMON_AUREALIS', 'SlthOnSummon', 'SLTH_UNIT_AUREALIS', 'SUN_THREE'),
('UNITOPERATION_SUMMON_PIT_BEAST', 'SlthOnSummon', 'SLTH_UNIT_PIT_BEAST', 'ENTROPY_TWO'),
('UNITOPERATION_SUMMON_MISTFORM', 'SlthOnSummon', 'SLTH_UNIT_MISTFORM', 'SHADOW_THREE'),
('UNITOPERATION_SUMMON_SPECTRE', 'SlthOnSummon', 'SLTH_UNIT_SPECTRE', 'DEATH_TWO'),
('UNITOPERATION_SUMMON_WATER_ELEMENTAL', 'SlthOnSummon', 'SLTH_UNIT_WATER_ELEMENTAL', 'WATER_THREE'),
('UNITOPERATION_SUMMON_WRAITH', 'SlthOnSummon', 'SLTH_UNIT_WRAITH', 'DEATH_THREE');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, PromotionPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_GRANT_STONESKIN_SELF', 'SlthOnGrantBuffSelf', 'BUFF_STONESKIN', 'EARTH_TWO', 'HasntAbility', '1'),
('UNITOPERATION_GRANT_WATERWALKING_SELF', 'SlthOnGrantBuffSelf', 'BUFF_WATERWALKING', 'WATER_TWO', 'HasntAbility', '1'),
('UNITOPERATION_GRANT_SPELL_STAFF_SELF', 'SlthOnGrantBuffSelf', 'SLTH_EQUIPMENT_SPELL_STAFF_ABILITY', 'ENCHANTMENT_THREE', 'HasntAbility', '1');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, PromotionPrereq, ActivationPrereq) VALUES
('UNITOPERATION_SUMMON_FLESH_GOLEM', 'SlthOnSummonPerm', 'SLTH_UNIT_FLESH_GOLEM', 'BODY_THREE', 'SingleSummon'),
('UNITOPERATION_SUMMON_SKELETON', 'SlthOnSummonPerm', 'SLTH_UNIT_SKELETON', 'DEATH_ONE', 'SingleSummon'),
('UNITOPERATION_GRANT_ENCHANTED_BLADE_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_ENCHANTED_BLADE', 'ENCHANTMENT_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_FAIR_WINDS', 'SlthOnGrantBuffAoEAlly', 'BUFF_FAIR_WINDS', 'AIR_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_HASTE_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_HASTE', 'BODY_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_REGENERATION_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_REGENERATION', 'BODY_TWO', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_DANCE_OF_BLADES_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_DANCE_OF_BLADES', 'CHAOS_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_FLAMING_ARROWS_AOE', 'SlthOnGrantBuffAoEAlly', 'ABILITY_FLAMING_ARROWS', 'ENCHANTMENT_TWO', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_LOYALTY_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_LOYALTY', 'LAW_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_VALOR_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_VALOR', 'LAW_THREE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_POISONED_BLADE_AOE', 'SlthOnGrantBuffAoEAlly', 'ABILITY_POISONED_BLADE', 'NATURE_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_TREETOP_DEFENSE_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_TREETOP_DEFENSE', 'NATURE_TWO', 'AdjacentAllyEligibleAbility'),         -- also needs feature ahh
('UNITOPERATION_GRANT_BLUR_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_BLUR', 'SHADOW_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_SHADOWWALK_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_SHADOWWALK', 'SHADOW_TWO', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_COURAGE_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_COURAGE', 'SPIRIT_ONE', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_GRANT_MUTATION_AOE', 'SlthOnGrantBuffAoEAlly', 'BUFF_MUTATED', 'CHAOS_TWO', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_DEBUFF_CHARMED_AOE', 'SlthOnGrantAbilityAoeEnemy', 'BUFF_CHARMED', 'MIND_TWO', 'AdjacentEnemyEligibleAbility'),
('UNITOPERATION_DEBUFF_RUSTED_AOE', 'SlthOnGrantAbilityAoeEnemy', 'BUFF_RUSTED', 'ENTROPY_ONE', 'AdjacentEnemyEligibleAbility'),
('UNITOPERATION_DEBUFF_BLINDED_AOE', 'SlthOnGrantAbilityAoeEnemy', 'BUFF_HELD', 'SUN_TWO', 'AdjacentEnemyEligibleAbility'),
('UNITOPERATION_DEBUFF_SLOW_AOE', 'SlthOnGrantAbilityAoeEnemy', 'BUFF_SLOW', 'ICE_ONE', 'AdjacentEnemyEligibleAbility'),
('UNITOPERATION_TERRAIN_CHANGE_SPRING', 'SlthOnChangeTerrain', 'SpringTo', 'WATER_ONE', 'DesertOrFlamesAdjacent'),
('UNITOPERATION_TERRAIN_CHANGE_SCORCH', 'SlthOnChangeTerrain', 'ScorchTo', 'SUN_ONE', 'OnSnowOrPlains'),
('UNITOPERATION_TERRAIN_CHANGE_VITALIZE', 'SlthOnChangeTerrain', 'VitalizeTo', 'NATURE_THREE', 'OnLandNotWoodedGrass'),
('UNITOPERATION_WONDER', 'SlthOnBespokeSpell', NULL, 'CHAOS_THREE', NULL),
('UNITOPERATION_BLAZE', 'SlthOnBespokeSpell', NULL, 'FIRE_ONE', 'OnForestOrJungle'),
('UNITOPERATION_RESURRECTION', 'SlthOnBespokeSpell', NULL, 'LIFE_THREE', 'PlayerHeroDead'),
('UNITOPERATION_DOMINATION', 'SlthOnBespokeSpell', NULL, 'MIND_THREE', 'AdjacentEnemyUnit'),             -- this needs targetting
('UNITOPERATION_TRUST', 'SlthOnBespokeSpell', NULL, 'SPIRIT_THREE', NULL),
('UNITOPERATION_SANCTIFY', 'SlthOnBespokeSpell', NULL, 'LIFE_ONE', 'OnCityRuinsOrGraveyardOrHellTerrain'),
('UNITOPERATION_DISPELL_MAGIC', 'SlthOnBespokeSpell', NULL, 'METAMAGIC_TWO', 'OnManaOrAdjacentUnitHasMagicDebuffOrBuff');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, PromotionPrereq, ActivationPrereq, SimpleAmount, SecondAmount, ThirdAmount) VALUES
('UNITOPERATION_AOE_DAMAGE_MAELSTROM', 'SlthOnAoeDamage', NULL, 'AIR_TWO', 'AdjacentEnemyUnit', 10, 40, 60),                    -- placeholder amounts for now
('UNITOPERATION_AOE_DAMAGE_DESTROY_UNDEAD', 'SlthOnAoeDamage', NULL, 'LIFE_TWO', 'AdjacentEnemyUnitIsUndead', 10, 40, 60),
('UNITOPERATION_AOE_DAMAGE_WITHER', 'SlthOnAoeDamage', NULL, 'ENTROPY_THREE', 'AdjacentEnemyUnit', 10, 40, 60),
('UNITOPERATION_AOE_DAMAGE_SNOWFALL', 'SlthOnAoeDamage', 'TERRAIN_SNOW', 'ICE_THREE', 'AdjacentEnemyUnit', 30, 50, 90);

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, PromotionPrereq, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_SUMMON_IRA', 'SlthOnSummonPerm', 'SLTH_UNIT_IRA', NULL, 'SLTH_UNIT_WRATH', NULL);

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, BuildingPrereq, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_LION_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_LION_CAGE', 'SLTH_UNIT_LION', 'SLTH_BUILDING_CARNIVAL', 'OnCityGrantBuilding'),
('UNITOPERATION_GRANT_WOLF_PEN', 'SlthOnGrantBuilding', 'SLTH_BUILDING_WOLF_PEN', 'SLTH_UNIT_WOLF', 'SLTH_BUILDING_CARNIVAL', 'OnCityGrantBuilding'),
('UNITOPERATION_GRANT_DANCING_BEAR', 'SlthOnGrantBuilding', 'SLTH_BUILDING_DANCING_BEAR', 'SLTH_UNIT_BEAR', NULL, 'OnCityGrantBuilding'),
('UNITOPERATION_GRANT_GORILLA_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_GORILLA_CAGE', 'SLTH_UNIT_GORILLA', 'SLTH_BUILDING_CARNIVAL', 'OnCityGrantBuilding'),
('UNITOPERATION_GRANT_SPIDER_PEN', 'SlthOnGrantBuilding', 'SLTH_BUILDING_SPIDER_PEN', 'SLTH_UNIT_GIANT_SPIDER', 'SLTH_BUILDING_CARNIVAL', 'OnCityGrantBuilding'),
('UNITOPERATION_GRANT_TIGER_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_TIGER_CAGE', 'SLTH_UNIT_TIGER', 'SLTH_BUILDING_CARNIVAL', 'OnCityGrantBuilding'),
('UNITOPERATION_GRANT_FREAK_SHOW', 'SlthOnGrantBuilding', 'SLTH_BUILDING_FREAK_SHOW', 'SLTH_UNIT_FREAK', NULL, 'OnCityGrantBuilding');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, BuildingPrereq, ActivationPrereq, TechPrereq) VALUES
('UNITOPERATION_GRANT_AQUEDUCT', 'SlthOnGrantBuilding', 'SLTH_BUILDING_AQUEDUCT', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', 'TECH_SANITATION'),
('UNITOPERATION_GRANT_ARCHERY_RANGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_ARCHERY_RANGE', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', 'SLTH_TECH_ARCHERY'),
('UNITOPERATION_GRANT_HUNTING_LODGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_HUNTING_LODGE', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', 'TECH_HUNTING'),
('UNITOPERATION_GRANT_LIBRARY', 'SlthOnGrantBuilding', 'BUILDING_LIBRARY', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', 'TECH_WRITING'),                  -- this does need a Campus..
('UNITOPERATION_GRANT_PALISADE', 'SlthOnGrantBuilding', 'BUILDING_WALLS', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', NULL),
('UNITOPERATION_GRANT_STABLE', 'SlthOnGrantBuilding', 'BUILDING_STABLE', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', 'TECH_HORSEBACK_RIDING'),
('UNITOPERATION_GRANT_TRAINING_YARD', 'SlthOnGrantBuilding', 'BUILDING_BARRACKS', 'SLTH_UNIT_SUPPLIES', NULL, 'OnCityGrantBuilding', 'TECH_BRONZE_WORKING'),
('UNITOPERATION_GRANT_ACADEMY', 'SlthOnGrantBuilding', 'BUILDING_ALCHEMICAL_SOCIETY', 'UNIT_GREAT_SCIENTIST', NULL, 'OnCityGrantBuilding', NULL);

-- Supplies
-- Aqueduct, Archery Range, Hunting Lodge, Library, Palisade, Stable, Training Yard

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_GRANT_BURNING_BLOOD', 'SlthOnGrantBuffSelf', 'BUFF_BURNING_BLOOD', 'SLTH_UNIT_MOROI', 'HasntAbility', '1');


INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_FEED_ON_CITY', 'SlthVampireConsumePop', NULL, 'ABILITY_VAMPIRISM', 'OnCityPopTwoPlus', '1');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_CONSUME_BLOODPET', 'SlthOnConsumeAlly', 'UNIT_WARRIOR', 'ABILITY_VAMPIRISM', 'AdjacentSingleAllyUnitMatches'),
('UNITOPERATION_OPEN_CHEST',       'SlthOnOpenChest', 'SLTH_EQUIPMENT_TREASURE', NULL, 'AdjacentSingleAllyUnitMatches');

-- ('UNITOPERATION_KIDNAP_GREAT_PERSON', 'LOC_KIDNAP_GREAT_PERSON_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT'),  todo hard plot property matches. but need to update superspecialist code

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, PromotionPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_REPAIR_GOLEM', 'SlthOnHealTargeted', 'GOLEM_RACE_LOW_HEALING_ABILITY', 'ENCHANTMENT_ONE', 'AdjacentSingleAllyIsDamagedAndGolem', '1');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_CONSUME_SOULS', 'SlthOnRefreshCasting', NULL, 'SLTH_UNIT_EATER_OF_DREAMS', 'OnCityPopTwoPlus');                               -- also need to ensure hasnt cast spells yet
INSERT INTO CustomOperations (OperationType, Callback, SimpleText, SecondText, AbilityPrereq, AlternateAbilityPrereq, BuildingPrereq, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_ELF_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_ELF_CAGE', 'SLTH_BUILDING_FREAK_SHOW', 'ELF_MOVEMENT_ON_FORESTED', 'ELF_RACIAL_MOVEMENT_ON_FOREST', 'SLTH_BUILDING_FREAK_SHOW', 'OnCityGrantBuildingPrereqBuilding'),        -- only capture inherent elves not racial. for that need two abilitiyies
('UNITOPERATION_GRANT_HUMAN_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_HUMAN_CAGE', 'SLTH_BUILDING_FREAK_SHOW', 'ABILITY_INHERENT_HUMAN', 'ABILITY_RACIAL_HUMAN','SLTH_BUILDING_FREAK_SHOW',  'OnCityGrantBuildingPrereqBuilding'),
('UNITOPERATION_GRANT_ORC_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_ORC_CAGE', 'SLTH_BUILDING_FREAK_SHOW', 'INHERENT_ORC_ABILITY_JUNGLE_ATTACKS', 'ORC_ABILITY_JUNGLE_ATTACKS','SLTH_BUILDING_FREAK_SHOW',  'OnCityGrantBuildingPrereqBuilding'),
('UNITOPERATION_GRANT_DWARF_CAGE', 'SlthOnGrantBuilding', 'SLTH_BUILDING_DWARF_CAGE', 'SLTH_BUILDING_FREAK_SHOW', 'INHERENT_DWARF_ABILITY_HILLS_DOUBLE_MOVE', 'DWARF_ABILITY_HILLS_DOUBLE_MOVE','SLTH_BUILDING_FREAK_SHOW',  'OnCityGrantBuildingPrereqBuilding');

INSERT INTO CustomOperations (OperationType, Callback, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_SUPER_SPECIALIST', 'SlthOnGrantSuperSpecialist', 'ABILITY_IS_GREAT_PERSON_NOT_COMMANDER', 'OnCityGeneric'),
('UNITOPERATION_GRANT_GOLDEN_AGE', 'SlthOnGrantGoldenAge', 'ABILITY_IS_GREAT_PERSON', 'EnoughGreatPeople');
-- EnoughGreatPeople

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, SecondText, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_CODE_OF_JUNIL', 'SlthOnGrantBuilding', 'SLTH_BUILDING_CODE_OF_JUNIL', 'RELIGION_PROTESTANTISM_HOLY_CITY', 'ABILITY_CAN_CODE_OF_JUNIL', 'OnHolyCity'),
('UNITOPERATION_GRANT_DIES_DEI', 'SlthOnGrantBuilding', 'BUILDING_ANGKOR_WAT', 'RELIGION_JUDAISM_HOLY_CITY', 'ABILITY_CAN_DIES_DEI', 'OnHolyCity'),
('UNITOPERATION_GRANT_TABLETS_OF_BAMBUR', 'SlthOnGrantBuilding', 'SLTH_BUILDING_TABLETS_OF_BAMBUR', 'RELIGION_CONFUCIANISM_HOLY_CITY', 'ABILITY_CAN_TABLETS', 'OnHolyCity'),
('UNITOPERATION_GRANT_SONG_OF_AUTUMN', 'SlthOnGrantBuilding', 'SLTH_BUILDING_SONG_OF_AUTUMN', 'RELIGION_CATHOLICISM_HOLY_CITY', 'ABILITY_CAN_SONG_OF_AUTUMN', 'OnHolyCity'),
('UNITOPERATION_GRANT_THE_NECRONOMICON', 'SlthOnGrantBuilding', 'SLTH_BUILDING_THE_NECRONOMICON', 'RELIGION_HINDUISM_HOLY_CITY', 'ABILITY_CAN_NECRONOMICON', 'OnHolyCity'),
('UNITOPERATION_GRANT_NOX_NOCTIS', 'SlthOnGrantBuilding', 'SLTH_BUILDING_NOX_NOCTIS', 'RELIGION_ISLAM_HOLY_CITY', 'ABILITY_CAN_NOX_NOCTIS', 'OnHolyCity'),
('UNITOPERATION_GRANT_STIGMATA_ON_THE_UNBORN', 'SlthOnGrantBuilding', 'SLTH_BUILDING_STIGMATA_ON_THE_UNBORN', 'RELIGION_BUDDHISM_HOLY_CITY', 'ABILITY_CAN_STIGMATA_ON_UNBORN', 'OnHolyCity');


INSERT INTO UnitAbilities(UnitAbilityType, Name, Description, Inactive, Permanent) VALUES
('ABILITY_IS_GREAT_PERSON', 'LOC_ABILITY_GREAT_PERSON_NAME', 'LOC_ABILITY_GREAT_PERSON_DESCRIPTION', '0', '1'),
('ABILITY_IS_GREAT_PERSON_NOT_COMMANDER', 'LOC_ABILITY_IS_GREAT_PERSON_NOT_COMMANDER_NAME', 'LOC_ABILITY_IS_GREAT_PERSON_NOT_COMMANDER_DESCRIPTION', '0', '1'),
('ABILITY_CAN_CODE_OF_JUNIL', 'LOC_CAN_CODE_OF_JUNIL_NAME', 'LOC_CAN_CODE_OF_JUNIL_DESCRIPTION', '0', '1'),
('ABILITY_CAN_DIES_DEI', 'LOC_CAN_DIES_DEI_NAME', 'LOC_CAN_DIES_DEI_DESCRIPTION', '0', '1'),
('ABILITY_CAN_TABLETS', 'LOC_CAN_TABLETS_NAME', 'LOC_CAN_TABLETS_DESCRIPTION', '0', '1'),
('ABILITY_CAN_NECRONOMICON', 'LOC_CAN_NECRONOMICON_NAME', 'LOC_CAN_NECRONOMICON_DESCRIPTION', '0', '1'),
('ABILITY_CAN_SONG_OF_AUTUMN', 'LOC_CAN_SONG_OF_AUTUMN_NAME', 'LOC_CAN_SONG_OF_AUTUMN_DESCRIPTION', '0', '1'),
('ABILITY_CAN_NOX_NOCTIS', 'LOC_CAN_NOX_NOCTIS_NAME', 'LOC_CAN_NOX_NOCTIS_DESCRIPTION', '0', '1'),
('ABILITY_CAN_STIGMATA_ON_UNBORN', 'LOC_CAN_STIGMATA_ON_UNBORN_NAME', 'LOC_CAN_STIGMATA_ON_UNBORN_DESCRIPTION', '0', '1'),
('ABILITY_MESMERISE_ANIMAL', 'LOC_MESMERISE_ANIMAL_NAME', 'LOC_MESMERISE_ANIMAL_DESCRIPTION', '0', '1'),
('ABILITY_MEDIC_3', 'LOC_MEDIC_3_NAME', 'LOC_MEDIC_3_DESCRIPTION', '0', '1');

INSERT INTO Tags(Tag, Vocabulary) VALUES
('CLASS_GREAT_PERSON', 'ABILITY_CLASS'),
('CLASS_GREAT_PERSON_NOT_COMMANDER', 'ABILITY_CLASS'),
('CLASS_CAN_JUNIL', 'ABILITY_CLASS'),
('CLASS_CAN_DIES_DEI', 'ABILITY_CLASS'),
('CLASS_CAN_TABLETS', 'ABILITY_CLASS'),
('CLASS_CAN_AUTUMN', 'ABILITY_CLASS'),
('CLASS_CAN_NOX', 'ABILITY_CLASS'),
('CLASS_CAN_STIGMATA_UNBORN', 'ABILITY_CLASS'),
('CLASS_CAN_NECRONOMICON', 'ABILITY_CLASS'),
('CLASS_FAWN', 'ABILITY_CLASS'),
('CLASS_MEDIC_3', 'ABILITY_CLASS');


INSERT INTO TypeTags(Type, Tag) VALUES
('ABILITY_IS_GREAT_PERSON', 'CLASS_GREAT_PERSON'),
('UNIT_GREAT_PROPHET', 'CLASS_GREAT_PERSON'),
('UNIT_GREAT_ENGINEER', 'CLASS_GREAT_PERSON'),
('UNIT_GREAT_SCIENTIST', 'CLASS_GREAT_PERSON'),
('UNIT_GREAT_ARTIST', 'CLASS_GREAT_PERSON'),
('UNIT_GREAT_MERCHANT', 'CLASS_GREAT_PERSON'),
('UNIT_GREAT_GENERAL', 'CLASS_GREAT_PERSON'),
('ABILITY_IS_GREAT_PERSON_NOT_COMMANDER', 'CLASS_GREAT_PERSON_NOT_COMMANDER'),
('UNIT_GREAT_PROPHET', 'CLASS_GREAT_PERSON_NOT_COMMANDER'),
('UNIT_GREAT_ENGINEER', 'CLASS_GREAT_PERSON_NOT_COMMANDER'),
('UNIT_GREAT_SCIENTIST', 'CLASS_GREAT_PERSON_NOT_COMMANDER'),
('UNIT_GREAT_ARTIST', 'CLASS_GREAT_PERSON_NOT_COMMANDER'),
('UNIT_GREAT_MERCHANT', 'CLASS_GREAT_PERSON_NOT_COMMANDER'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_JUNIL'),
('UNIT_GREAT_GENERAL', 'CLASS_CAN_JUNIL'),
('ABILITY_CAN_CODE_OF_JUNIL', 'CLASS_CAN_JUNIL'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_DIES_DEI'),
('UNIT_GREAT_SCIENTIST', 'CLASS_CAN_DIES_DEI'),
('ABILITY_CAN_DIES_DEI', 'CLASS_CAN_DIES_DEI'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_TABLETS'),
('UNIT_GREAT_ENGINEER', 'CLASS_CAN_TABLETS'),
('ABILITY_CAN_TABLETS', 'CLASS_CAN_TABLETS'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_AUTUMN'),
('UNIT_GREAT_ARTIST', 'CLASS_CAN_AUTUMN'),
('ABILITY_CAN_SONG_OF_AUTUMN', 'CLASS_CAN_AUTUMN'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_NOX'),
('UNIT_GREAT_MERCHANT', 'CLASS_CAN_NOX'),
('ABILITY_CAN_NOX_NOCTIS', 'CLASS_CAN_NOX'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_STIGMATA_UNBORN'),
('UNIT_GREAT_SCIENTIST', 'CLASS_CAN_STIGMATA_UNBORN'),
('ABILITY_CAN_STIGMATA_ON_UNBORN', 'CLASS_CAN_STIGMATA_UNBORN'),
('ABILITY_CAN_NECRONOMICON', 'CLASS_CAN_NECRONOMICON'),
('UNIT_GREAT_PROPHET', 'CLASS_CAN_NECRONOMICON'),
('ABILITY_MEDIC_3', 'CLASS_MEDIC_3'),
('SLTH_UNIT_SPHENER', 'CLASS_MEDIC_3'),
('SLTH_UNIT_YVAIN', 'CLASS_MEDIC_3'),
('SLTH_UNIT_GRIGORI_MEDIC', 'CLASS_MEDIC_3'),
('SLTH_UNIT_HIGH_PRIEST_OF_KILMORPH', 'CLASS_MEDIC_3'),
('SLTH_UNIT_HIGH_PRIEST_OF_THE_OVERLORDS', 'CLASS_MEDIC_3'),
('SLTH_UNIT_HIGH_PRIEST_OF_THE_EMPYREAN', 'CLASS_MEDIC_3'),
('SLTH_UNIT_HIGH_PRIEST_OF_THE_ORDER', 'CLASS_MEDIC_3'),
('SLTH_UNIT_HIGH_PRIEST_OF_THE_VEIL', 'CLASS_MEDIC_3'),
('SLTH_UNIT_HIGH_PRIEST_OF_LEAVES', 'CLASS_MEDIC_3'),
('ABILITY_MESMERISE_ANIMAL', 'CLASS_FAWN'),
('SLTH_UNIT_FAWN', 'CLASS_FAWN'),
('SLTH_UNIT_SATYR', 'CLASS_FAWN');

INSERT INTO Types(Type, Kind) VALUES
('ABILITY_IS_GREAT_PERSON', 'KIND_ABILITY'),
('ABILITY_IS_GREAT_PERSON_NOT_COMMANDER', 'KIND_ABILITY'),
('ABILITY_CAN_CODE_OF_JUNIL', 'KIND_ABILITY'),
('ABILITY_CAN_DIES_DEI', 'KIND_ABILITY'),
('ABILITY_CAN_TABLETS', 'KIND_ABILITY'),
('ABILITY_CAN_SONG_OF_AUTUMN', 'KIND_ABILITY'),
('ABILITY_CAN_NOX_NOCTIS', 'KIND_ABILITY'),
('ABILITY_CAN_STIGMATA_ON_UNBORN', 'KIND_ABILITY'),
('ABILITY_CAN_NECRONOMICON', 'KIND_ABILITY'),
('ABILITY_MESMERISE_ANIMAL', 'KIND_ABILITY'),
('ABILITY_MEDIC_3', 'KIND_ABILITY');


INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_CAST_MIRROR', 'SlthOnCastMirror', NULL, 'SLTH_EQUIPMENT_BLACK_MIRROR_ABILITY', NULL),
('UNITOPERATION_READ_GRIMOIRE', 'SlthOnBespokeSpell', NULL, 'SLTH_EQUIPMENT_INFERNAL_GRIMOIRE_ABILITY', NULL),
('UNITOPERATION_BREAK_STAFF', 'SlthOnBreakStaff', NULL, 'SLTH_EQUIPMENT_SPELL_STAFF_ABILITY', NULL),
('UNITOPERATION_SPREAD_ESUS', 'SlthOnSpreadEsus', NULL, 'ABILITY_WORSHIPS_ESUS', 'OnAdjacentCity');                -- todo make adjacent so can do enemy cities

INSERT INTO CustomOperations (OperationType, Callback, UnitPrereq) VALUES
('UNITOPERATION_ESCAPE', 'SlthOnTeleportToCapital', 'SLTH_UNIT_CHANTER');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_SEVER_SOUL', 'SlthOnSummonPerm', 'SLTH_UNIT_SEVERED_SOUL', 'SLTH_UNIT_DIVIDED_SOUL', 'SingleSummon'),                --TODO implement severed soul unit
('UNITOPERATION_CALL_FORM', 'SlthOnTeleportToSummon', 'SLTH_UNIT_SEVERED_SOUL', 'SLTH_UNIT_DIVIDED_SOUL', 'HasSummon');
INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq) VALUES
('UNITOPERATION_SUMMON_PUPPET', 'SlthOnSummon', 'SLTH_UNIT_PUPPET', 'ABILITY_CAN_PUPPET');

       -- Hope, Inspiration, Wall of Stone. just do REQUIREMENT_PLOT_ADJACENT_TO_OWNER? and ability on unit? PLAYER_CITIES Modifier?
    -- 	['LOC_UNITOPERATION_BUILDING_DESCRIPTION'] = {OnStart=OnRemoveBuff, iAmount=50},  FEATURE_BURNING_FOREST
    -- Special: Resurrection, Domination, Trust
-- }

-- ADJACENT UNITType MATCHES: Consume Bloodpet, Repair Golem
INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_HERALDS_CALL', 'SlthOnGrantAbilityTargeted', 'BUFF_HERALDS_CALL', 'SLTH_UNIT_HERALD', 'AdjacentAllyEligibleAbility');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_MORALE', 'SlthOnGrantBuffAoEAlly', 'BUFF_MORALE', 'SLTH_UNIT_FLAGBEARER', 'AdjacentAllyEligibleAbility'),
('UNITOPERATION_TEACH_SPELLS', 'SlthOnGrantPromotionAoEAlly', NULL, 'SLTH_UNIT_GOVANNON', 'AdjacentUnitCanHavePromoGovannonHas'),     -- grants promotions based on govannon.
('UNITOPERATION_ENTANGLE', 'SlthOnGrantAbilityTargetedEnemy', 'BUFF_HELD', 'SLTH_UNIT_DRUID', 'AdjacentEnemyEligibleAbility'),              -- placeholder abilities
('UNITOPERATION_SPORES', 'SlthOnGrantAbilityAoeEnemy', 'ABILITY_SLEPT', 'SLTH_UNIT_MYCONID', 'AdjacentEnemyEligibleAbility'),
('UNITOPERATION_TAUNT', 'SlthOnGrantAbilityTargetedEnemy', 'BUFF_TAUNT', 'SLTH_UNIT_HARLEQUIN', 'AdjacentEnemyEligibleAbility');

INSERT INTO CustomOperations (OperationType, Callback, SimpleAmount, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_CRUSH', 'SlthOnDamageTargeted',   80, 'SLTH_UNIT_DWARVEN_DRUID', 'AdjacentEnemyUnit'),
('UNITOPERATION_HASTURS_RAZOR', 'SlthOnAoeDamage', 90, 'SLTH_UNIT_HEMAH', 'AdjacentEnemyUnit'),
('UNITOPERATION_PILLAR_OF_FIRE', 'SlthOnDamageTargeted', 90, 'SLTH_UNIT_CHALID', 'AdjacentEnemyUnit');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_MESMERISE_ANIMAL', 'SlthOnConvertUnit', 'PROMOTION_CLASS_ANIMAL', 'ABILITY_MESMERISE_ANIMAL', 'AdjacentEnemyEligiblePromoClass'),
('UNITOPERATION_HEAL_FULL_ALLY', 'SlthOnHealTargeted', NULL, 'ABILITY_MEDIC_3', 'AdjacentSingleAllyIsDamaged');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, SimpleAmount, ActivationPrereq) VALUES
('UNITOPERATION_GRANT_VAMPIRISM', 'SlthOnGrantAbilityTargeted', 'ABILITY_VAMPIRISM', 'ABILITY_VAMPIRISM', 6, 'AdjacentSingleAllyIsLevelMinimum');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, SimpleAmount, AbilityPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_BECOME_SHADE', 'SlthOnConvertSelf', 'SLTH_UNIT_SHADE', 6, 'ABILITY_SIDAR_GRAY', 'UnitIsLevel', '1');

INSERT INTO CustomOperations (OperationType, Callback, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_ABSORB_UNIT', 'SlthOnConsumeAlly', 'SLTH_UNIT_FLESH_GOLEM', 'AdjacentSingleAlly');         -- hard to do

INSERT INTO CustomOperations (OperationType, Callback, BuildingPrereq, FormationPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_ARENA_FIGHT', 'SlthOnUnitCityInteract', 'BUILDING_ARENA', 'FORMATION_CLASS_LAND_COMBAT', 'AdjacentCityHasBuilding', '1'),
('UNITOPERATION_SACRIFICE_ALTAR', 'SlthOnUnitCityInteract', 'SLTH_BUILDING_DEMONS_ALTAR', 'FORMATION_CLASS_LAND_COMBAT', 'AdjacentCityHasBuilding', '1');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, BuildingPrereq, UnitPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_DROWN_WARRIOR', 'SlthOnConvertSelf', 'SLTH_UNIT_DROWN', 'BUILDING_MOSQUE', 'UNIT_WARRIOR', 'AdjacentCityHasBuilding', '1');

INSERT INTO CustomOperations (OperationType, Callback, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_KIDNAP_GREAT_PERSON', 'SlthOnBespokeSpell', 'SLTH_ABILITY_SINISTER', 'AdjacentCityHasPlotPropertyAndWarPossible');       -- todo need to do more to catch all the GPP type PlotProperties and also rework that work

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_SUMMON_SAND_LION', 'SlthOnSummon', 'SLTH_UNIT_SAND_LION', 'ABILITY_SAND_LION_SUMMONER', 'OnDesertTerrain');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, UnitPrereq, ActivationPrereq) VALUES
('UNITOPERATION_LOKI_DISRUPT_CITY', 'SlthOnUnitCityInteract', NULL, 'SLTH_UNIT_LOKI', 'OnAdjacentEnemyCity'),
('UNITOPERATION_LOKI_ENTERTAIN_CITY', 'SlthOnUnitCityInteract', NULL, 'SLTH_UNIT_LOKI', 'OnAdjacentCity'),
('UNITOPERATION_CORLINDALE_PEACE', 'SlthOnForcePeace', NULL, 'SLTH_UNIT_CORLINDALE', 'AtWar');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_PLACE_BARNAXUS_PIECES', 'SlthOnUnitCityInteract', '', 'SLTH_EQUIPMENT_PIECES_OF_BARNAXUS_ABILITY', 'OnCityGeneric'),             -- this seems painful
('UNITOPERATION_PLACE_GRIMOIRE', 'SlthOnUnitCityInteract', 'MODIFIER_GRANT_INFERNAL_GRIMOIRE_BUILDING', 'SLTH_EQUIPMENT_INFERNAL_GRIMOIRE_ABILITY', 'OnCityGeneric'),
('UNITOPERATION_PLACE_CROWN', 'SlthOnUnitCityInteract', 'MODIFIER_GRANT_CROWN_OF_AKHARIEN_BUILDING', 'SLTH_EQUIPMENT_CROWN_OF_AKHARIEN_ABILITY', 'OnCityGeneric'),
('UNITOPERATION_PLACE_LYRE', 'SlthOnUnitCityInteract', 'MODIFIER_GRANT_SYLIVENS_PERFECT_LYRE_BUILDING', 'SLTH_EQUIPMENT_SYLIVENS_PERFECT_LYRE_ABILITY', 'OnCityGeneric'),
('UNITOPERATION_PLACE_HORDE', 'SlthOnUnitCityInteract', 'MODIFIER_GRANT_THE_DRAGONS_HORDE_BUILDING', 'SLTH_EQUIPMENT_DRAGONS_HORDE_ABILITY', 'OnCityGeneric'),
('UNITOPERATION_PLACE_CAULDRON', 'SlthOnUnitCityInteract', 'MODIFIER_GRANT_MOKKAS_CAULDRON_BUILDING', 'SLTH_EQUIPMENT_MOKKA_CAULDRON_ABILITY', 'OnCityGeneric');

INSERT INTO CustomOperations (OperationType, Callback, DomainPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_TAKE_EQUIPMENT', 'SlthOnTakeEquipment', 'DOMAIN_LAND', 'AdjacentSingleAllyHasEquipmentOrIsEquipment', '1');

INSERT INTO CustomOperations (OperationType, Callback, BuildingPrereq, ActivationPrereq) VALUES
('UNITOPERATION_HEAL_SIRONA', 'SlthOnHealSelf', 'BUILDING_CRISTO_REDENTOR', 'IsDamaged');                   -- needs condition of once per turn

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, SecondText, PromotionPrereq, ActivationPrereq, DontShowOnDisabled) VALUES
('UNITOPERATION_BECOME_LICH', 'SlthOnConvertSelf', 'SLTH_UNIT_LICH', 'ABILITY_UNDEAD', 'DEATH_THREE', 'HasntAbility', '1');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, AlsoAbilityPrereq, AlsoTwoAbilityPrereq) VALUES
('UNITOPERATION_SUMMON_BALOR', 'SlthOnSummonPerm', 'SLTH_UNIT_BALOR',  'ABILITY_DIVINE', 'ABILITY_CHANNELING3', 'ABILITY_WORSHIPS_VEIL');

INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, AlsoAbilityPrereq, AlsoTwoAbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_SUMMON_KRAKEN', 'SlthOnSummonPerm', 'SLTH_UNIT_KRAKEN', 'ABILITY_DIVINE', 'ABILITY_CHANNELING3', 'ABILITY_WORSHIPS_OCTOPUS', 'SingleSummonAtSea'),
('UNITOPERATION_SUMMON_TREANT', 'SlthOnSummonPerm', 'SLTH_UNIT_TREANT', 'ABILITY_DIVINE', 'ABILITY_CHANNELING3', 'ABILITY_WORSHIPS_LEAVES', 'SingleSummonOnForest'),
('UNITOPERATION_EARTHQUAKE', 'SlthOnAoeDamage', NULL, 'ABILITY_DIVINE', 'ABILITY_CHANNELING3', 'ABILITY_WORSHIPS_KILMORPH', 'AdjacentEnemyUnit');


INSERT INTO CustomOperations (OperationType, Callback, SimpleText, AbilityPrereq, AlsoAbilityPrereq, AlsoTwoAbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_SUMMON_TIGER', 'SlthOnSummonPerm', 'SLTH_UNIT_TIGER', 'ABILITY_DIVINE', 'ABILITY_CHANNELING2', 'ABILITY_WORSHIPS_LEAVES', 'SingleSummon'),
('UNITOPERATION_BLOOM', 'SlthOnMakeFeature', 'FEATURE_FOREST_ANCIENT', 'ABILITY_DIVINE', 'ABILITY_CHANNELING2', 'ABILITY_WORSHIPS_LEAVES', 'HasntFeature'),
('UNITOPERATION_REVELATION', 'SlthOnBespokeSpell', NULL, 'ABILITY_DIVINE', 'ABILITY_CHANNELING2', 'ABILITY_WORSHIPS_EMPYREAN', NULL),
('UNITOPERATION_GRANT_SHIELD_OF_FAITH', 'SlthOnGrantBuffAoEAlly', 'BUFF_SHIELD_OF_FAITH', 'ABILITY_DIVINE', 'ABILITY_CHANNELING2', 'ABILITY_WORSHIPS_KILMORPH', 'AdjacentAllyEligibleAbility');

INSERT INTO CustomOperations (OperationType, Callback, SimpleAmount, AbilityPrereq, AlsoAbilityPrereq, AlsoTwoAbilityPrereq, ActivationPrereq) VALUES
('UNITOPERATION_TSUNAMI', 'SlthOnAoeDamageCoast', 50, 'ABILITY_DIVINE', 'ABILITY_CHANNELING2', 'ABILITY_WORSHIPS_OCTOPUS', 'AdjacentEnemyUnitAtSea'),
('UNITOPERATION_RING_OF_FLAMES', 'SlthOnAoeDamage', 30, 'ABILITY_DIVINE', 'ABILITY_CHANNELING2', 'ABILITY_WORSHIPS_VEIL', 'AdjacentEnemyUnit');


-- Divine spells (Has abilityPrereq, AlternateAbilityPrereq): CoE steal equipment,


-- crown of brilliance not done as unitcommand, its more like a passive.
-- Create Hellfire, not done as no feature currently, from Profane.

-- Open Chest(just extend equipment pickup), Sprint(Centaur units), healing salve
-- low prio: Khazad create Battering Ram (it sucks lol), Impersonate Leader
-- Impersonate Leader. Completely impossible lmao.

-- AI only: Call of the Grave (ars), Rage (Buboes)
-- cant remember, how it works Doviello Duels
-- Ships change Crews. Only one crew does something not involving embarked units. Shelve this
-- Magnadine (hire, made obselete by vi). Hide (grant unit camoflauge, why would i remove.),

CREATE TABLE IF NOT EXISTS TerrainTransforms(
    TerrainType text primary key,
    VitalizeTo text,
    ScorchTo text,
    SpringTo text,
    HellTerrainVariant text,
    IsHellTerrain boolean);

INSERT INTO TerrainTransforms (TerrainType, VitalizeTo, ScorchTo, SpringTo, HellTerrainVariant, IsHellTerrain) VALUES
('TERRAIN_BURNING_SANDS', 'TERRAIN_BROKEN_LANDS', NULL, 'TERRAIN_BROKEN_LANDS', 'TERRAIN_DESERT', 1),
('TERRAIN_BURNING_SANDS_HILLS', 'TERRAIN_BROKEN_LANDS_HILLS', NULL,'TERRAIN_BROKEN_LANDS_HILLS', 'TERRAIN_DESERT_HILLS', 1),
('TERRAIN_FIELDS_OF_PERDITION', 'TERRAIN_BROKEN_LANDS', 'TERRAIN_BURNING_SANDS', NULL, 'TERRAIN_PLAINS', 1),
('TERRAIN_FIELDS_OF_PERDITION_HILLS', 'TERRAIN_BROKEN_LANDS_HILLS', 'TERRAIN_BURNING_SANDS_HILLS', NULL, 'TERRAIN_PLAINS_HILLS', 1),
('TERRAIN_BROKEN_LANDS', NULL, NULL, NULL, 'TERRAIN_GRASS', 1),
('TERRAIN_BROKEN_LANDS_HILLS', NULL, NULL,  NULL, 'TERRAIN_GRASS_HILLS', 1),
('TERRAIN_DESERT', 'TERRAIN_PLAINS', NULL, 'TERRAIN_PLAINS','TERRAIN_BURNING_SANDS', 0),
('TERRAIN_DESERT_HILLS', 'TERRAIN_PLAINS_HILLS', NULL, 'TERRAIN_PLAINS_HILLS', 'TERRAIN_BURNING_SANDS_HILLS', 0),
('TERRAIN_PLAINS', 'TERRAIN_GRASS', 'TERRAIN_DESERT', NULL, 'TERRAIN_FIELDS_OF_PERDITION', 0),
('TERRAIN_PLAINS_HILLS', 'TERRAIN_GRASS_HILLS', 'TERRAIN_DESERT_HILLS',NULL, 'TERRAIN_FIELDS_OF_PERDITION_HILLS', 0),
('TERRAIN_GRASS', NULL, NULL, NULL, 'TERRAIN_BROKEN_LANDS', 0),
('TERRAIN_GRASS_HILLS', NULL, NULL, NULL, 'TERRAIN_BROKEN_LANDS_HILLS', 0),
('TERRAIN_TUNDRA', 'TERRAIN_PLAINS', 'TERRAIN_PLAINS', NULL, NULL, 0),
('TERRAIN_TUNDRA_HILLS', 'TERRAIN_PLAINS_HILLS', 'TERRAIN_PLAINS_HILLS', NULL, NULL, 0),
('TERRAIN_SNOW', 'TERRAIN_TUNDRA', 'TERRAIN_TUNDRA', NULL, NULL, 0),
('TERRAIN_SNOW_HILLS', 'TERRAIN_TUNDRA_HILLS',  'TERRAIN_TUNDRA_HILLS', NULL, NULL, 0);

CREATE TABLE IF NOT EXISTS ResourceTransforms(
    ResourceType text primary key ,
    ConvertedResource text,
    ConvertedResourceVariant text,
    IsHellResource boolean);

INSERT INTO ResourceTransforms (ResourceType, ConvertedResource, ConvertedResourceVariant, IsHellResource) VALUES
('RESOURCE_TRUFFLES', 'RESOURCE_TOAD', NULL, 0),
('RESOURCE_SHEEP', 'RESOURCE_TOAD', NULL, 0),
('RESOURCE_TOAD', 'RESOURCE_TRUFFLES', 'RESOURCE_SHEEP', 1),
('RESOURCE_CATTLE', 'RESOURCE_NIGHTMARE', NULL, 0),
('RESOURCE_HORSES', 'RESOURCE_NIGHTMARE', NULL, 0),
('RESOURCE_NIGHTMARE', 'RESOURCE_CATTLE', 'RESOURCE_HORSES', 1),
('RESOURCE_MARBLE', 'RESOURCE_JADE', NULL, 0),
('RESOURCE_JADE', 'RESOURCE_MARBLE', NULL, 1),
('RESOURCE_BANANAS', 'RESOURCE_TOBACCO', NULL, 0),
('RESOURCE_SUGAR', 'RESOURCE_TOBACCO', NULL, 0),
('RESOURCE_TOBACCO', 'RESOURCE_BANANAS', 'RESOURCE_SUGAR', 1),
('RESOURCE_SILK', 'RESOURCE_OLIVES', NULL, 0),
('RESOURCE_COTTON', 'RESOURCE_OLIVES', NULL, 0),
('RESOURCE_OLIVES', 'RESOURCE_SILK', 'RESOURCE_COTTON', 1);


INSERT INTO UnitOperations (OperationType, Description, Icon, VisibleInUI, HoldCycling, CategoryInUI, Sound) VALUES
('UNITOPERATION_SUMMON_AIR_ELEMENTAL', 'LOC_SUMMON_AIR_ELEMENTAL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_AIR_ELEMENTAL', 1, 1, 'BUILD', 'Play_Airelemental'),
('UNITOPERATION_SUMMON_DJINN', 'LOC_SUMMON_DJINN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_DJINN', 1, 1, 'BUILD', 'Play_Spring'),
('UNITOPERATION_SUMMON_EYE', 'LOC_SUMMON_EYE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_EYE', 1, 1, 'BUILD', 'Play_Kraken'),
('UNITOPERATION_SUMMON_EARTH_ELEMENTAL', 'LOC_SUMMON_EARTH_ELEMENTAL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_EARTH_ELEMENTAL', 1, 1, 'BUILD', 'Play_Earthelemental'),
('UNITOPERATION_SUMMON_FIRE_ELEMENTAL', 'LOC_SUMMON_FIRE_ELEMENTAL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_FIRE_ELEMENTAL', 1, 1, 'BUILD', 'Play_Fireelemental'),
('UNITOPERATION_SUMMON_FIREBALL', 'LOC_SUMMON_FIREBALL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_FIREBALL', 1, 1, 'BUILD', 'Play_Fireball'),
('UNITOPERATION_SUMMON_ICE_ELEMENTAL', 'LOC_SUMMON_ICE_ELEMENTAL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_ICE_ELEMENTAL', 1, 1, 'BUILD', 'Play_Spring'),
('UNITOPERATION_SUMMON_EINHERJAR', 'LOC_SUMMON_EINHERJAR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_EINHERJAR', 1, 1, 'BUILD', 'Play_Einherjar'),
('UNITOPERATION_SUMMON_AUREALIS', 'LOC_SUMMON_AUREALIS_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_AUREALIS', 1, 1, 'BUILD', 'Play_Scorch'),
('UNITOPERATION_SUMMON_PIT_BEAST', 'LOC_SUMMON_PIT_BEAST_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_PIT_BEAST', 1, 1, 'BUILD', 'Play_Pitbeast'),
('UNITOPERATION_SUMMON_MISTFORM', 'LOC_SUMMON_MISTFORM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_MISTFORM', 1, 1, 'BUILD', 'Play_Kraken'),           -- placeholder
('UNITOPERATION_SUMMON_SPECTRE', 'LOC_SUMMON_SPECTRE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_SPECTRE', 1, 1, 'BUILD', 'Play_Spectre'),
('UNITOPERATION_SUMMON_WATER_ELEMENTAL', 'LOC_SUMMON_WATER_ELEMENTAL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_WATER_ELEMENTAL', 1, 1, 'BUILD', 'Play_Spring'),
('UNITOPERATION_SUMMON_WRAITH', 'LOC_SUMMON_WRAITH_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_WRAITH', 1, 1, 'BUILD', 'Play_Wraith'),
('UNITOPERATION_SUMMON_FLESH_GOLEM', 'LOC_SUMMON_FLESH_GOLEM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_FLESH_GOLEM', 1, 1, 'BUILD', 'Play_Graftflesh'),
('UNITOPERATION_SUMMON_SKELETON', 'LOC_SUMMON_SKELETON_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_SKELETON', 1, 1, 'BUILD', 'Play_Summon_Skeleton'),     --todo
('UNITOPERATION_SUMMON_IRA', 'LOC_SUMMON_IRA_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_IRA', 1, 1, 'BUILD', 'Play_Fireelemental'),
('UNITOPERATION_GRANT_STONESKIN_SELF', 'LOC_GRANT_STONESKIN_SELF_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_STONESKIN_SELF', 1, 1, 'BUILD', 'Play_Stoneskin'),
('UNITOPERATION_GRANT_WATERWALKING_SELF', 'LOC_GRANT_WATERWALKING_SELF_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_WATERWALKING_SELF', 1, 1, 'BUILD', 'Play_Water'),
('UNITOPERATION_GRANT_SPELL_STAFF_SELF', 'LOC_GRANT_SPELL_STAFF_SELF_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_SPELL_STAFF_SELF', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_GRANT_ENCHANTED_BLADE_AOE', 'LOC_GRANT_ENCHANTED_BLADE_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_ENCHANTED_BLADE_AOE', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_GRANT_FAIR_WINDS', 'LOC_GRANT_FAIR_WINDS_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_FAIR_WINDS', 1, 1, 'BUILD', 'Play_Haste'),
('UNITOPERATION_GRANT_HASTE_AOE', 'LOC_GRANT_HASTE_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_HASTE_AOE', 1, 1, 'BUILD', 'Play_Haste'),
('UNITOPERATION_GRANT_REGENERATION_AOE', 'LOC_GRANT_REGENERATION_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_REGENERATION_AOE', 1, 1, 'BUILD', 'Play_Bless'),
('UNITOPERATION_GRANT_DANCE_OF_BLADES_AOE', 'LOC_GRANT_DANCE_OF_BLADES_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_DANCE_OF_BLADES_AOE', 1, 1, 'BUILD', 'Play_Haste'),
('UNITOPERATION_GRANT_FLAMING_ARROWS_AOE', 'LOC_GRANT_FLAMING_ARROWS_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_FLAMING_ARROWS_AOE', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_GRANT_LOYALTY_AOE', 'LOC_GRANT_LOYALTY_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_LOYALTY_AOE', 1, 1, 'BUILD', 'Play_Bless'),
('UNITOPERATION_GRANT_VALOR_AOE', 'LOC_GRANT_VALOR_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_VALOR_AOE', 1, 1, 'BUILD', 'Play_Bless'),
('UNITOPERATION_GRANT_POISONED_BLADE_AOE', 'LOC_GRANT_POISONED_BLADE_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_POISONED_BLADE_AOE', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_GRANT_TREETOP_DEFENSE_AOE', 'LOC_GRANT_TREETOP_DEFENSE_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_TREETOP_DEFENSE_AOE', 1, 1, 'BUILD', 'Play_Sanctify'),
('UNITOPERATION_GRANT_BLUR_AOE', 'LOC_GRANT_BLUR_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_BLUR_AOE', 1, 1, 'BUILD', 'Play_Charmperson'),
('UNITOPERATION_GRANT_SHADOWWALK_AOE', 'LOC_GRANT_SHADOWWALK_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_SHADOWWALK_AOE', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_GRANT_COURAGE_AOE', 'LOC_GRANT_COURAGE_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_COURAGE_AOE', 1, 1, 'BUILD', 'Play_Bless'),
('UNITOPERATION_GRANT_MUTATION_AOE', 'LOC_GRANT_MUTATION_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_MUTATION_AOE', 1, 1, 'BUILD', 'Play_Mutation'),
('UNITOPERATION_DEBUFF_CHARMED_AOE', 'LOC_DEBUFF_CHARMED_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DEBUFF_CHARMED_AOE', 1, 1, 'BUILD', 'Play_Charmperson'),
('UNITOPERATION_DEBUFF_RUSTED_AOE', 'LOC_DEBUFF_RUSTED_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DEBUFF_RUSTED_AOE', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_DEBUFF_BLINDED_AOE', 'LOC_DEBUFF_BLINDED_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DEBUFF_BLINDED_AOE', 1, 1, 'BUILD', 'Play_Charmperson'),
('UNITOPERATION_DEBUFF_SLOW_AOE', 'LOC_DEBUFF_SLOW_AOE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DEBUFF_SLOW_AOE', 1, 1, 'BUILD', 'Play_Spring'),
('UNITOPERATION_TERRAIN_CHANGE_SPRING', 'LOC_TERRAIN_CHANGE_SPRING_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TERRAIN_CHANGE_SPRING', 1, 1, 'BUILD', 'Play_Spring'),
('UNITOPERATION_TERRAIN_CHANGE_SCORCH', 'LOC_TERRAIN_CHANGE_SCORCH_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TERRAIN_CHANGE_SCORCH', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_TERRAIN_CHANGE_VITALIZE', 'LOC_TERRAIN_CHANGE_VITALIZE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TERRAIN_CHANGE_VITALIZE', 1, 1, 'BUILD', 'Play_Sanctify'),
('UNITOPERATION_WONDER', 'LOC__WONDER_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_WONDER', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_BLAZE', 'LOC__BLAZE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_BLAZE', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_RESURRECTION', 'LOC__RESURRECTION_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_RESURRECTION', 1, 1, 'BUILD', 'Play_Curedisease'),
('UNITOPERATION_TRUST', 'LOC__TRUST_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TRUST', 1, 1, 'BUILD', 'Play_Hope'),
('UNITOPERATION_SANCTIFY', 'LOC__SANCTIFY_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SANCTIFY', 1, 1, 'BUILD', 'Play_Sanctify'),
('UNITOPERATION_DISPELL_MAGIC', 'LOC__DISPELL_MAGIC_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DISPELL_MAGIC', 1, 1, 'BUILD', 'Play_Dispelmagic'),
('UNITOPERATION_FEED_ON_CITY', 'LOC_FEED_ON_CITY_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_FEED_ON_CITY', 1, 1, 'BUILD', 'Play_Feast'),
('UNITOPERATION_GRANT_LION_CAGE', 'LOC_GRANT_LION_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_LION_CAGE', 1, 1, 'BUILD', 'Play_Foundtemple'),
('UNITOPERATION_GRANT_WOLF_PEN', 'LOC_GRANT_WOLF_PEN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_WOLF_PEN', 1, 1, 'BUILD', 'Play_Foundtemple'),
('UNITOPERATION_GRANT_DANCING_BEAR', 'LOC_GRANT_DANCING_BEAR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_DANCING_BEAR', 1, 1, 'BUILD', 'Play_Foundtemple'),
('UNITOPERATION_GRANT_GORILLA_CAGE', 'LOC_GRANT_GORILLA_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_GORILLA_CAGE', 1, 1, 'BUILD', 'Play_Foundtemple'),
('UNITOPERATION_GRANT_SPIDER_PEN', 'LOC_GRANT_SPIDER_PEN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_SPIDER_PEN', 1, 1, 'BUILD', 'Play_Foundtemple'),      -- there may be a silk in units
('UNITOPERATION_GRANT_TIGER_CAGE', 'LOC_GRANT_TIGER_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_TIGER_CAGE', 1, 1, 'BUILD', 'Play_Tiger'),
('UNITOPERATION_GRANT_FREAK_SHOW', 'LOC_GRANT_FREAK_SHOW_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_FREAK_SHOW', 1, 1, 'BUILD', 'Play_Carnival'),
('UNITOPERATION_GRANT_BURNING_BLOOD', 'LOC_GRANT_BURNING_BLOOD_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_BURNING_BLOOD', 1, 1, 'BUILD', 'Play_Burningblood'),
('UNITOPERATION_CONSUME_SOULS', 'LOC_CONSUME_SOULS_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_CONSUME_SOULS', 1, 1, 'BUILD', 'Play_Consumesoul'),
('UNITOPERATION_GRANT_ELF_CAGE', 'LOC_GRANT_ELF_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_ELF_CAGE', 1, 1, 'BUILD', 'Play_Dispelmagic'),
('UNITOPERATION_GRANT_HUMAN_CAGE', 'LOC_GRANT_HUMAN_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_HUMAN_CAGE', 1, 1, 'BUILD', 'Play_Dispelmagic'),
('UNITOPERATION_GRANT_ORC_CAGE', 'LOC_GRANT_ORC_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_ORC_CAGE', 1, 1, 'BUILD', 'Play_Dispelmagic'),
('UNITOPERATION_GRANT_DWARF_CAGE', 'LOC_GRANT_DWARF_CAGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_DWARF_CAGE', 1, 1, 'BUILD', 'Play_Dispelmagic'),
('UNITOPERATION_GRANT_SUPER_SPECIALIST', 'LOC_GRANT_SUPER_SPECIALIST_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_SUPER_SPECIALIST', 1, 1, 'BUILD', 'Play_Commanderjoin'),            -- new
('UNITOPERATION_GRANT_GOLDEN_AGE', 'LOC_GRANT_GOLDEN_AGE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_GOLDEN_AGE', 1, 1, 'BUILD', 'Play_Commanderjoin'),            -- new should be bell age
('UNITOPERATION_GRANT_CODE_OF_JUNIL', 'LOC_GRANT_CODE_OF_JUNIL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_CODE_OF_JUNIL', 1, 1, 'BUILD', 'Play_Order'),     -- not done in same way, default found
('UNITOPERATION_GRANT_DIES_DEI', 'LOC_GRANT_DIES_DEI_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_DIES_DEI', 1, 1, 'BUILD', 'Play_Empyrean'),
('UNITOPERATION_GRANT_TABLETS_OF_BAMBUR', 'LOC_GRANT_TABLETS_OF_BAMBUR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_TABLETS_OF_BAMBUR', 1, 1, 'BUILD', 'Play_Runes_of_Kilmorph'),
('UNITOPERATION_GRANT_SONG_OF_AUTUMN', 'LOC_GRANT_SONG_OF_AUTUMN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_SONG_OF_AUTUMN', 1, 1, 'BUILD', 'Play_Fellowship_of_Leaves'),
('UNITOPERATION_GRANT_THE_NECRONOMICON', 'LOC_GRANT_THE_NECRONOMICON_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_THE_NECRONOMICON', 1, 1, 'BUILD', 'Play_Octopus_Overlords'),
('UNITOPERATION_GRANT_NOX_NOCTIS', 'LOC_GRANT_NOX_NOCTIS_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_NOX_NOCTIS', 1, 1, 'BUILD', 'Play_Council_of_Esus'),
('UNITOPERATION_GRANT_STIGMATA_ON_THE_UNBORN', 'LOC_GRANT_STIGMATA_ON_THE_UNBORN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_STIGMATA_ON_THE_UNBORN', 1, 1, 'BUILD', 'Play_Ashen_Veil'),
('UNITOPERATION_CAST_MIRROR', 'LOC_CAST_MIRROR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_CAST_MIRROR', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_READ_GRIMOIRE', 'LOC_READ_GRIMOIRE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_READ_GRIMOIRE', 1, 1, 'BUILD', 'Play_Train'),
('UNITOPERATION_BREAK_STAFF', 'LOC_BREAK_STAFF_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_BREAK_STAFF', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_ESCAPE', 'LOC_ESCAPE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_ESCAPE', 1, 1, 'BUILD', 'Play_Escape'),
('UNITOPERATION_SEVER_SOUL', 'LOC_SEVER_SOUL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SEVER_SOUL', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_CALL_FORM', 'LOC_CALL_FORM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_CALL_FORM', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_SUMMON_PUPPET', 'LOC_SUMMON_PUPPET_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_PUPPET', 1, 1, 'BUILD', 'Play_Create_Puppet'),
('UNITOPERATION_GRANT_MORALE', 'LOC_GRANT_MORALE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_MORALE', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_TEACH_SPELLS', 'LOC_TEACH_SPELLS_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TEACH_SPELLS', 1, 1, 'BUILD', 'Play_Train'),
('UNITOPERATION_SPORES', 'LOC_SPORES_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SPORES', 1, 1, 'BUILD', 'Play_Contagion'),
('UNITOPERATION_MESMERISE_ANIMAL', 'LOC_MESMERISE_ANIMAL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_MESMERISE_ANIMAL', 1, 1, 'BUILD', 'Play_Charmperson'),
('UNITOPERATION_BECOME_SHADE', 'LOC_BECOME_SHADE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_BECOME_SHADE', 1, 1, 'BUILD', 'Play_UnitRises'),          --new todo?
('UNITOPERATION_ARENA_FIGHT', 'LOC_ARENA_FIGHT_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_ARENA_FIGHT', 1, 1, 'BUILD', 'Play_Charmperson'),
('UNITOPERATION_SACRIFICE_ALTAR', 'LOC_SACRIFICE_ALTAR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SACRIFICE_ALTAR', 1, 1, 'BUILD', 'Play_Sacrifice'),
('UNITOPERATION_DROWN_WARRIOR', 'LOC_DROWN_WARRIOR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DROWN_WARRIOR', 1, 1, 'BUILD', 'Play_Drown'),
('UNITOPERATION_SUMMON_SAND_LION', 'LOC_SUMMON_SAND_LION_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_SAND_LION', 1, 1, 'BUILD', 'Play_Tiger'),
('UNITOPERATION_CORLINDALE_PEACE', 'LOC_CORLINDALE_PEACE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_CORLINDALE_PEACE', 1, 1, 'BUILD', 'Play_Curedisease'),
('UNITOPERATION_PLACE_BARNAXUS_PIECES', 'LOC_PLACE_BARNAXUS_PIECES_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PLACE_BARNAXUS_PIECES', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_PLACE_GRIMOIRE', 'LOC_PLACE_GRIMOIRE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PLACE_GRIMOIRE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_PLACE_CROWN', 'LOC_PLACE_CROWN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PLACE_CROWN', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_PLACE_LYRE', 'LOC_PLACE_LYRE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PLACE_LYRE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_HEAL_SIRONA', 'LOC_HEAL_SIRONA_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_HEAL_SIRONA', 1, 1, 'BUILD', 'Play_Heal'),
('UNITOPERATION_BECOME_LICH', 'LOC_BECOME_LICH_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_BECOME_LICH', 1, 1, 'BUILD', 'Play_Defile'),
('UNITOPERATION_SUMMON_BALOR', 'LOC_SUMMON_BALOR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_BALOR', 1, 1, 'BUILD', 'Play_balorSelect'),
('UNITOPERATION_SUMMON_KRAKEN', 'LOC_SUMMON_KRAKEN_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_KRAKEN', 1, 1, 'BUILD', 'Play_Kraken'),
('UNITOPERATION_SUMMON_TREANT', 'LOC_SUMMON_TREANT_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_TREANT', 1, 1, 'BUILD', 'Play_Bloom'),
('UNITOPERATION_SUMMON_TIGER', 'LOC_SUMMON_TIGER_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SUMMON_TIGER', 1, 1, 'BUILD', 'Play_Tiger'),
('UNITOPERATION_BLOOM', 'LOC_BLOOM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_BLOOM', 1, 1, 'BUILD', 'Play_Bloom'),
('UNITOPERATION_REVELATION', 'LOC_REVELATION_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_REVELATION', 1, 1, 'BUILD', 'Play_Revelation'),
('UNITOPERATION_GRANT_SHIELD_OF_FAITH', 'LOC_GRANT_SHIELD_OF_FAITH_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_SHIELD_OF_FAITH', 1, 1, 'BUILD', 'Play_Enchantment'),
('UNITOPERATION_PLACE_HORDE', 'LOC_PLACE_HORDE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PLACE_HORDE', 1, 1, 'BUILD', 'Play_Repair'),      -- todo
('UNITOPERATION_PLACE_CAULDRON', 'LOC_PLACE_CAULDRON_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PLACE_CAULDRON', 1, 1, 'BUILD', 'Play_Repair'),

('UNITOPERATION_GRANT_AQUEDUCT', 'LOC_GRANT_AQUEDUCT_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_ARCHERY_RANGE', 'LOC_GRANT_ARCHERY_RANGE_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_HUNTING_LODGE', 'LOC_GRANT_HUNTING_LODGE_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_LIBRARY', 'LOC_GRANT_LIBRARY_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_PALISADE', 'LOC_GRANT_PALISADE_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_STABLE', 'LOC_GRANT_STABLE_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_TRAINING_YARD', 'LOC_GRANT_TRAINING_YARD_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Repair'),
('UNITOPERATION_GRANT_ACADEMY', 'LOC_GRANT_ACADEMY_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD', 'Play_Carnival');

-- ('UNITOPERATION_$', 'LOC_$_DESCRIPTION', 'ICON_UNITCOMMAND_TREAT_WITH_CLAN_DISPERSE', 1, 1, 'BUILD'),


INSERT INTO UnitOperations (OperationType, Description, Icon, VisibleInUI, HoldCycling, CategoryInUI, Sound) VALUES
('UNITOPERATION_AOE_DAMAGE_MAELSTROM', 'LOC_AOE_DAMAGE_MAELSTROM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_AOE_DAMAGE_MAELSTROM', 1, 1, 'BUILD', 'Oya_Storm_2D'),
('UNITOPERATION_AOE_DAMAGE_DESTROY_UNDEAD', 'LOC_AOE_DAMAGE_DESTROY_UNDEAD_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_AOE_DAMAGE_DESTROY_UNDEAD', 1, 1, 'BUILD', 'Play_Destroyundead'),
('UNITOPERATION_AOE_DAMAGE_WITHER', 'LOC_AOE_DAMAGE_WITHER_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_AOE_DAMAGE_WITHER', 1, 1, 'BUILD', 'Play_Consumesoul'),
('UNITOPERATION_AOE_DAMAGE_SNOWFALL', 'LOC_AOE_DAMAGE_SNOWFALL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_AOE_DAMAGE_SNOWFALL', 1, 1, 'BUILD', 'Play_Spring'),      -- right, but could be cooler For such a cool effect
('UNITOPERATION_HASTURS_RAZOR', 'LOC_HASTURS_RAZOR_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_HASTURS_RAZOR', 1, 1, 'BUILD', 'Play_Mutation'),
('UNITOPERATION_EARTHQUAKE', 'LOC_EARTHQUAKE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_EARTHQUAKE', 1, 1, 'BUILD', 'Play_Earthquake'),
('UNITOPERATION_TSUNAMI', 'LOC_TSUNAMI_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TSUNAMI', 1, 1, 'BUILD', 'Play_Tsunami'),
('UNITOPERATION_RING_OF_FLAMES', 'LOC_RING_OF_FLAMES_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_RING_OF_FLAMES', 1, 1, 'BUILD', 'Play_Fireball');


-- TARGETED
INSERT INTO UnitOperations (OperationType, Description, Icon, VisibleInUI, HoldCycling, CategoryInUI, InterfaceMode, Sound) VALUES
('UNITOPERATION_CONSUME_BLOODPET', 'LOC_CONSUME_BLOODPET_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_CONSUME_BLOODPET', 1, 1, 'BUILD', 'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Feed'),
('UNITOPERATION_OPEN_CHEST', 'LOC_OPEN_CHEST_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_OPEN_CHEST', 1, 1, 'BUILD', 'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Enchantment'),
('UNITOPERATION_HEAL_FULL_ALLY', 'LOC_HEAL_FULL_ALLY_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_HEAL_FULL_ALLY', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Heal'),
('UNITOPERATION_GRANT_VAMPIRISM', 'LOC_GRANT_VAMPIRISM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_VAMPIRISM', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Giftvampirism'),
('UNITOPERATION_KIDNAP_GREAT_PERSON', 'LOC_KIDNAP_GREAT_PERSON_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_KIDNAP_GREAT_PERSON', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Drown'),      -- was bless, but thats silly
('UNITOPERATION_ABSORB_UNIT', 'LOC_ABSORB_UNIT_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_ABSORB_UNIT', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Graftflesh'),
('UNITOPERATION_LOKI_DISRUPT_CITY', 'LOC_LOKI_DISRUPT_CITY_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_LOKI_DISRUPT_CITY', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Feast'),      -- revolt non-existing
('UNITOPERATION_LOKI_ENTERTAIN_CITY', 'LOC_LOKI_ENTERTAIN_CITY_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_LOKI_ENTERTAIN_CITY', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Carnival'),            -- crowdcheer, base civ iv. use some celebration sound? ugh
('UNITOPERATION_TAUNT', 'LOC_TAUNT_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TAUNT', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Sacrifice'),
('UNITOPERATION_DOMINATION', 'LOC_DOMINATION_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_DOMINATION', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Domination'),
('UNITOPERATION_REPAIR_GOLEM', 'LOC_REPAIR_GOLEM_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_REPAIR_GOLEM', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Repair'),
('UNITOPERATION_SPREAD_ESUS', 'LOC_SPREAD_ESUS_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_SPREAD_ESUS', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Council_of_Esus'),
('UNITOPERATION_GRANT_HERALDS_CALL', 'LOC_HERALDS_CALL_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_GRANT_HERALDS_CALL', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Hope'),
('UNITOPERATION_ENTANGLE', 'LOC_ENTANGLE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_ENTANGLE', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Bloom'),
('UNITOPERATION_CRUSH', 'LOC_CRUSH_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_CRUSH', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Earthquake'),
('UNITOPERATION_PILLAR_OF_FIRE', 'LOC_PILLAR_OF_FIRE_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_PILLAR_OF_FIRE', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Fireball'),
('UNITOPERATION_TAKE_EQUIPMENT', 'LOC_TAKE_EQUIPMENT_DESCRIPTION', 'ICON_SLTH_UNITOPERATION_TAKE_EQUIPMENT', 1, 1, 'BUILD',  'INTERFACEMODE_WB_SELECT_PLOT', 'Play_Train');

INSERT INTO Types(Type, Kind) VALUES
('UNITOPERATION_SUMMON_AIR_ELEMENTAL', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_DJINN', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_EYE', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_EARTH_ELEMENTAL', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_FIRE_ELEMENTAL', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_FIREBALL', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_EINHERJAR', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_AUREALIS', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_PIT_BEAST', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_MISTFORM', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_SPECTRE', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_WATER_ELEMENTAL', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_WRAITH', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_FLESH_GOLEM', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_SKELETON', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_IRA', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_ICE_ELEMENTAL', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_STONESKIN_SELF', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_WATERWALKING_SELF', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_SPELL_STAFF_SELF', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_ENCHANTED_BLADE_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_FAIR_WINDS', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_HASTE_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_REGENERATION_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_DANCE_OF_BLADES_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_FLAMING_ARROWS_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_LOYALTY_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_VALOR_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_POISONED_BLADE_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_TREETOP_DEFENSE_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_BLUR_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_SHADOWWALK_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_COURAGE_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_MUTATION_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_DEBUFF_CHARMED_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_DEBUFF_RUSTED_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_DEBUFF_BLINDED_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_DEBUFF_SLOW_AOE', 'KIND_UNITOPERATION'),
('UNITOPERATION_TERRAIN_CHANGE_SPRING', 'KIND_UNITOPERATION'),
('UNITOPERATION_TERRAIN_CHANGE_SCORCH', 'KIND_UNITOPERATION'),
('UNITOPERATION_TERRAIN_CHANGE_VITALIZE', 'KIND_UNITOPERATION'),
('UNITOPERATION_AOE_DAMAGE_MAELSTROM', 'KIND_UNITOPERATION'),
('UNITOPERATION_AOE_DAMAGE_DESTROY_UNDEAD', 'KIND_UNITOPERATION'),
('UNITOPERATION_AOE_DAMAGE_WITHER', 'KIND_UNITOPERATION'),
('UNITOPERATION_AOE_DAMAGE_SNOWFALL', 'KIND_UNITOPERATION'),
('UNITOPERATION_WONDER', 'KIND_UNITOPERATION'),
('UNITOPERATION_BLAZE', 'KIND_UNITOPERATION'),
('UNITOPERATION_RESURRECTION', 'KIND_UNITOPERATION'),
('UNITOPERATION_DOMINATION', 'KIND_UNITOPERATION'),
('UNITOPERATION_TRUST', 'KIND_UNITOPERATION'),
('UNITOPERATION_SANCTIFY', 'KIND_UNITOPERATION'),
('UNITOPERATION_DISPELL_MAGIC', 'KIND_UNITOPERATION'),
('UNITOPERATION_FEED_ON_CITY', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_LION_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_WOLF_PEN', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_DANCING_BEAR', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_GORILLA_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_SPIDER_PEN', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_TIGER_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_FREAK_SHOW', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_BURNING_BLOOD', 'KIND_UNITOPERATION'),
('UNITOPERATION_CONSUME_SOULS', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_ELF_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_HUMAN_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_ORC_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_DWARF_CAGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_SUPER_SPECIALIST', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_GOLDEN_AGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_CODE_OF_JUNIL', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_DIES_DEI', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_TABLETS_OF_BAMBUR', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_SONG_OF_AUTUMN', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_THE_NECRONOMICON', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_NOX_NOCTIS', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_STIGMATA_ON_THE_UNBORN', 'KIND_UNITOPERATION'),
('UNITOPERATION_CONSUME_BLOODPET', 'KIND_UNITOPERATION'),
('UNITOPERATION_REPAIR_GOLEM', 'KIND_UNITOPERATION'),
('UNITOPERATION_CAST_MIRROR', 'KIND_UNITOPERATION'),
('UNITOPERATION_READ_GRIMOIRE', 'KIND_UNITOPERATION'),
('UNITOPERATION_BREAK_STAFF', 'KIND_UNITOPERATION'),
('UNITOPERATION_SPREAD_ESUS', 'KIND_UNITOPERATION'),
('UNITOPERATION_ESCAPE', 'KIND_UNITOPERATION'),
('UNITOPERATION_SEVER_SOUL', 'KIND_UNITOPERATION'),
('UNITOPERATION_CALL_FORM', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_PUPPET', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_HERALDS_CALL', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_MORALE', 'KIND_UNITOPERATION'),
('UNITOPERATION_TEACH_SPELLS', 'KIND_UNITOPERATION'),
('UNITOPERATION_ENTANGLE', 'KIND_UNITOPERATION'),
('UNITOPERATION_SPORES', 'KIND_UNITOPERATION'),
('UNITOPERATION_CRUSH', 'KIND_UNITOPERATION'),
('UNITOPERATION_HASTURS_RAZOR', 'KIND_UNITOPERATION'),
('UNITOPERATION_PILLAR_OF_FIRE', 'KIND_UNITOPERATION'),
('UNITOPERATION_MESMERISE_ANIMAL', 'KIND_UNITOPERATION'),
('UNITOPERATION_HEAL_FULL_ALLY', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_VAMPIRISM', 'KIND_UNITOPERATION'),
('UNITOPERATION_BECOME_SHADE', 'KIND_UNITOPERATION'),
('UNITOPERATION_ABSORB_UNIT', 'KIND_UNITOPERATION'),
('UNITOPERATION_ARENA_FIGHT', 'KIND_UNITOPERATION'),
('UNITOPERATION_SACRIFICE_ALTAR', 'KIND_UNITOPERATION'),
('UNITOPERATION_DROWN_WARRIOR', 'KIND_UNITOPERATION'),
('UNITOPERATION_KIDNAP_GREAT_PERSON', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_SAND_LION', 'KIND_UNITOPERATION'),
('UNITOPERATION_LOKI_DISRUPT_CITY', 'KIND_UNITOPERATION'),
('UNITOPERATION_LOKI_ENTERTAIN_CITY', 'KIND_UNITOPERATION'),
('UNITOPERATION_CORLINDALE_PEACE', 'KIND_UNITOPERATION'),
('UNITOPERATION_PLACE_BARNAXUS_PIECES', 'KIND_UNITOPERATION'),
('UNITOPERATION_PLACE_GRIMOIRE', 'KIND_UNITOPERATION'),
('UNITOPERATION_PLACE_CROWN', 'KIND_UNITOPERATION'),
('UNITOPERATION_PLACE_LYRE', 'KIND_UNITOPERATION'),
('UNITOPERATION_HEAL_SIRONA', 'KIND_UNITOPERATION'),
('UNITOPERATION_BECOME_LICH', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_BALOR', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_KRAKEN', 'KIND_UNITOPERATION'),
('UNITOPERATION_EARTHQUAKE', 'KIND_UNITOPERATION'),
('UNITOPERATION_SUMMON_TIGER', 'KIND_UNITOPERATION'),
('UNITOPERATION_BLOOM', 'KIND_UNITOPERATION'),
('UNITOPERATION_REVELATION', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_SHIELD_OF_FAITH', 'KIND_UNITOPERATION'),
('UNITOPERATION_TSUNAMI', 'KIND_UNITOPERATION'),
('UNITOPERATION_RING_OF_FLAMES', 'KIND_UNITOPERATION'),
('UNITOPERATION_PLACE_HORDE', 'KIND_UNITOPERATION'),
('UNITOPERATION_PLACE_CAULDRON', 'KIND_UNITOPERATION'),
('UNITOPERATION_TAKE_EQUIPMENT', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_AQUEDUCT', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_ARCHERY_RANGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_HUNTING_LODGE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_LIBRARY',  'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_PALISADE', 'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_STABLE',  'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_TRAINING_YARD',  'KIND_UNITOPERATION'),
('UNITOPERATION_GRANT_ACADEMY',  'KIND_UNITOPERATION'),
('UNITOPERATION_OPEN_CHEST',  'KIND_UNITOPERATION');

